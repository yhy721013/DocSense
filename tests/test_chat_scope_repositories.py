...
from app.modules.chat.adapters.sqlite import repositories as chat_repositories
...
class ChatScopeRepositoryTests(unittest.TestCase):
    ...
    def _create_run(self, *, conversation_id: str, run_id: str):
        """为仓储测试直接建立 Conversation 与 run，不经过身份准入用例。"""
        self.store.sessions.create_or_get(conversation_id=conversation_id)
        if self.store.session_scope_bindings.get(conversation_id) is None:
    ...
    def test_current_scope_tables_columns_and_migrations_are_present(self) -> None:
        ...
        # Schema v9 将删除审计从在线 Conversation 聚合中解耦，属于当前结构的一部分。
        # 这里显式覆盖 1..9，避免以后新增迁移后测试仍错误地接受旧版本上限。
        self.assertEqual(set(range(1, 10)), versions)
        self.assertTrue(
            {
        ...
        self.assertIn("conversation_id", revision_foreign_keys)
        self.assertNotIn("source_run_id", revision_foreign_keys)

    # 根据已批准的 conversation-v1 清库重建决策，旧 v4-v6 原地迁移测试已删除；
    # 旧世代数据库失败关闭的行为由仓储 Schema 测试单独覆盖。
    ...
    def test_fifty_architecture_bindings_are_isolated(self) -> None:
        ...
        def create_binding(index: int) -> tuple[str, int]:
            ...
            self.store.sessions.create_or_get(conversation_id=chat_id)
            ...
            return binding.conversation_id, int(binding.architecture_id)
    ...
    def test_append_revision_reads_ordered_members_and_head(self) -> None:
        self._create_run(conversation_id="chat-a", run_id="run-a")
    ...
    def test_scope_head_compare_and_set_keeps_append_only_history(self) -> None:
        first_run = self._create_run(conversation_id="chat-a", run_id="run-a")
        ...
        self.store.runs.mark_running(first_run.run_id)
        self.store.runs.mark_succeeded(first_run.run_id)
        second_run = self._create_run(conversation_id="chat-a", run_id="run-b")
    ...
    def test_empty_scope_revision_is_persisted_as_real_head(self) -> None:
        run = self._create_run(conversation_id="chat-empty", run_id="run-empty")
        revision = self._revision(
    ...
    def test_revision_source_run_must_belong_to_same_chat(self) -> None:
        self._create_run(conversation_id="chat-a", run_id="run-a")
        self._create_run(conversation_id="chat-b", run_id="run-b")
    ...
    def test_revision_and_members_are_update_immutable(self) -> None:
        run = self._create_run(conversation_id="chat-a", run_id="run-a")
        revision = self._revision(
            conversation_id="chat-a",
            run_id=run.run_id,
            file_names=("a.pdf",),
    ...
    def test_fifty_distinct_chats_keep_scope_facts_isolated(self) -> None:
        ...
        def create_scope(index: int) -> tuple[str, str]:
            ...
            run_id = f"run-{index}"
            barrier.wait(timeout=10)
            self._create_run(conversation_id=chat_id, run_id=run_id)
            revision = self._revision(
                conversation_id=chat_id,
                run_id=run_id,
