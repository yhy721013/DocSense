...
class Stage1DWeaponryContractAssetTests(unittest.TestCase):
    ...
    def test_progress_and_check_task_are_public_contract_extensions_without_new_fields(self) -> None:
        """Weaponry 已公开支持两条辅助入口，但不增加任何载荷字段。"""
        ...
        self.assertEqual("public-contract-active", boundary["progress"]["status"])
        self.assertEqual("public-contract-active", boundary["checkTask"]["status"])
        self.assertEqual(
            "must-remain",
