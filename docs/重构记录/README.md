...
| `260718-阶段1H共享文档处理模块文件级实施设计.md` | L3 已完成文件级设计：1H-0～1H-7 已完成通用 Artifact/Profile/Lineage、共享 DocumentProcessing Application、独立 Translation 模块、逐调用方切换、永久门禁和关闭引用矩阵；当前生产执行权已切换，但兼容源码和测试引用仍待 1G 按证据处理，阶段 3 再切换 MinIO。 |
| `260801-阶段1G组合根Debug与遗留清理文件级实施设计.md` | L3 待实施文件级设计：按 1G-0～1G-6 冻结契约与引用基线、建立 Debug Query/Port/Adapter、分离框架无关组合根、关闭正式蓝图边界、迁移遗留测试价值并条件删除旧执行器；物理删除必须等待仓库内引用清零、仓库外调用与运行配置确认及回滚观察期结束。 |
