...
| `260730-文件分析RAG内部Markdown投影与业务原名上传命名实施计划.md` | L3 已确认、待实施专项计划：为文件分析生成不含图片 Base64 payload 的 RAG-only Markdown Artifact；multipart filename 使用 `<originalFileName主干>.md` 控制 Markdown 解析，AnythingLLM metadata.title、UI 与 Chunk sourceDocument 使用带原后缀的 `originalFileName`。本地 Artifact 继续使用任务隔离和内容身份；缺失/空 originalFileName 回退 fileName，非法名称整批同步 HTTP 400，真实 PDF 降级保持 PDF。 |
