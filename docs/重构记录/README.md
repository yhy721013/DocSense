...
| `260804-知识谱系解析直接父节点文件范围实施计划.md` | L3 待实施专项：在不增删公开接口参数的前提下，由 Analysis 受理事务把已验证 `parentId` 原子投影为本地父关系目录，并由 ENV 开关控制 `/llm/weaponry` 空文件范围是否冻结“请求存储类别 + 直接父类别”文档。2026-08-09 已确认仅影响该接口的缺省/空 `filePathList`、非空显式范围不扩展、`architectureId` 精确表示 `documents.architecture_id`，且授权 Analysis 调用方的已验证 `parentId` 是唯一父关系来源；默认树发布、冲突收敛、公开失败合同、启用覆盖门禁及修订审计仍须在阶段 0 确认。 |
...
| `260803-永久知识谱系Workspace命名实施计划.md` | L3 阶段 0～7 已完成专项：Analysis 入库和 Reassign 目标准备共用的永久知识谱系 Workspace 名称已从 `architectureId-{id}` 收口为 `archId-{规范化id}`；不兼容迁移旧数据，不改变 `/llm/weaponry` 临时 Workspace 或任何公开接口参数；离线门禁及本机回环 AnythingLLM 的隔离创建、核名和清理均已通过。 |
| `260803-知识谱系对话来源Metadata清洗实施计划.md` | L3 阶段 0～7 已完成专项：经负责人授权更新既有 `content` 内容语义，在 Weaponry Chat 来源公开映射边界删除 Chunk 开头完整的 AnythingLLM `<document_metadata>` 包装，并令 SSE 与 History 原子复用同一清洗快照；畸形包装失败关闭，不增删接口参数、不影响文件对话、不迁移旧数据。离线安全全仓及本机 AnythingLLM、Flask 协议级真实验收已通过，证据不构成浏览器 UI、生产多实例、可靠队列或容量证明。 |
...
| `260730-文件分析RAG内部Markdown投影与业务原名上传命名实施计划.md` | L3 已完成专项计划：P0～P9 已落地 RAG-only Markdown Artifact、Provider 中立上传描述符及资源/审计事实；v2 投影删除 Base64 图片完整语法且不生成提示或图片元数据，只保留 Token 分隔空格。multipart filename 使用全局唯一 `fileName` 主干，AnythingLLM metadata.title、UI 与 Chunk sourceDocument 使用带原后缀的业务原名；真实 PDF 降级保持 PDF，尚未部署生产环境。 |
...
| `260718-阶段1H共享文档处理模块文件级实施设计.md` | L3 已完成文件级设计：1H-0～1H-7 已完成通用 Artifact/Profile/Lineage、共享 DocumentProcessing Application、独立 Translation 模块、逐调用方切换、永久门禁和关闭引用矩阵；当前生产执行权已切换，兼容源码与测试引用已由 1G 完成价值迁移和条件物理删除，阶段 3 再切换 MinIO。 |
| `260801-阶段1G组合根Debug与遗留清理文件级实施设计.md` | L3 已完成文件级设计：1G-0～1G-6 已完成契约/引用基线、Debug 分层、框架无关组合根、正式蓝图编排下沉、测试价值迁移、条件物理删除、所有权矩阵和九组关闭验收；安全全仓发现 2,118、排除 13、执行 2,105 项且失败/错误为 0，下一步进入阶段 2。 |
