...
| `260728-知识谱系类别文件对话详细实施计划.md` | L3 待实施计划：在现有 `/llm/chat` 中增加与 fileNames 隔离的 architecture 模式，以不可变会话绑定、直接文件首轮快照、Schema v5、原子受理、恢复和历史字段互斥完成纵向闭环。 |
| `260728-知识谱系类别文件对话详细集成计划.md` | L3 待执行集成计划：规定功能分支提交序列、主线漂移处理、Schema v5 迁移演练、定向/全仓门禁、前后端上线顺序和完整回滚。 |
