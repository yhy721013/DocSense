...
| `260803-知识谱系对话来源Metadata清洗实施计划.md` | L3 待实施专项：计划在 Weaponry Chat 来源映射边界删除 Chunk 开头完整的 AnythingLLM `<document_metadata>` 包装，并令 SSE 与 History 原子复用同一清洗快照；不增删接口参数、不影响文件对话、不迁移旧数据。因现行合同冻结了“上游字符串完全不变”，阶段 0 必须先取得接口文档语义更新授权。 |
