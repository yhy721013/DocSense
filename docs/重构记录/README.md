...
| `260715-统一任务最小契约与Progress语义预设计.md` | L3 跨阶段契约：统一任务、回调恢复、保守投递策略和 Progress 内部语义。 |
| `260715-阶段1A-1B文件级实施设计.md` | L3 文件级设计：首个实施切片的文件落点、调用链、测试、日志、回滚和完成门禁。 |
...
- 任何接口文档变更都必须先取得确认；任何情况下都不得擅自增删前后端请求参数。2026-07-15 已批准的成功空响应体、report 409 和 Progress 显式 action/ack 下线仅是定向例外，不得扩张解释。
