...
| `260804-知识谱系解析直接父节点文件范围实施计划.md` | L3 待实施专项：在不增删公开接口参数的前提下，由 Analysis 受理事务把已验证 `parentId` 原子投影为本地父关系目录，并由 ENV 开关控制 `/llm/weaponry` 空文件范围是否冻结“目标类别 + 直接父类别”文档；显式 `filePathList` 不扩展，历史 execution 不扫描 `path` 猜父级，未知或冲突关系失败关闭。正式实施前须先确认接口范围与失败语义。 |
