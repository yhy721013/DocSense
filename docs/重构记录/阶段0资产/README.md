...
| 容量采集器 | `scripts/stage0_load_baseline.py` | 连接已有服务采集 HTTP/SSE/WS 成功率、吞吐和分位延迟；不启动服务 |
| 容量场景 | `scripts/stage0_workloads.example.json` | 定义 50 短请求、50 WS、禁用的 50 SSE 与在途任务场景 |
| SQLite 盘点器 | `scripts/stage0_sqlite_inventory.py` | 使用 `mode=ro` 与 `query_only` 输出 Schema/聚合，不迁移数据 |
