...
| 阶段 1H 调用点与资源矩阵 | `docs/重构记录/阶段0资产/260729-阶段1H现状调用点与资源所有权矩阵.md` | 保存共享文档处理迁移前事实及关闭增量 |
| 阶段 1 关闭所有权矩阵 | `docs/重构记录/阶段0资产/260801-阶段1关闭模块所有权与遗留适配矩阵.md` | 固化当前模块/数据库表所有权、依赖图、遗留处置、回滚点与阶段 2 输入 |
| 阶段 1 关闭资产门禁 | `tests/test_stage1g_closeout.py` | 防止新表无所有者、模块说明缺失或已删除旧运行路径回流 |
