...
Bootstrap 具有两个互斥入口：存在旧权威事实时只能执行 6.1 的迁移预检；旧文件集已经由受控维护
彻底弃用时，只能执行 6.3 的显式 fresh 初始化。普通应用启动不得根据“路径恰好不存在”自行猜测
fresh，也不得创建空旧库来冒充已通过迁移预检。
...
### 6.3 旧文件集已弃用的显式 fresh 初始化

2026-08-20 用户确认采用以下一次性部署策略：

1. fresh 只由独立运维命令调用，必须携带显式确认；普通应用 Container 不自动触发；
2. 在同一 Schema 锁内证明旧路径和 v2 目标的主文件、`-wal/-shm/-journal` 均不存在；
3. 任一旧文件残留、目标残留、锁争用或确认缺失都失败关闭，不自动删除、归档或覆盖；
4. 使用 6.1 相同的临时建库、全部必需组件安装、严格验证和不覆盖原子发布算法；
5. fresh 成功后应用按 6.2 打开既有 v2；重复执行 fresh 必须拒绝，而不是把它当普通 reopen；
6. 该确认只代表历史控制面事实已被明确放弃，不代表浏览器、外部供应商、多实例或生产容量验收。

当前旧 SQLite 仍临时承载 Processing Record、知识索引协调、RAG 租约、可观测审计和 Terms 状态等
非 Task 表。fresh 发布后允许应用为了这些未迁移职责创建一份新的空辅助库，但 Report、Weaponry、
Analysis 的 Task/Callback 控制事实只写 v2；物理拆库和旧 Schema 删除留到阶段 2-8。
...
- 至少区分：路径冲突、旧库预检阻塞、目标文件集冲突、数据库身份错误、根 generation 错误、
  ...
  busy/locked、fresh 确认缺失、旧文件残留和初始化发布竞态；日志仅记录错误分类及
  路径/UUID/fingerprint 的短摘要。
...
11. fresh 初始化确认缺失、旧文件任一成员残留、目标任一成员残留、重复执行和锁争用全部拒绝；
    成功发布后完整组件集合一次就绪，且普通启动只走严格打开。
