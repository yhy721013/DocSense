...
| 文档状态 | 实施中；阶段 0 已按已确认决策关闭，阶段 1 已于 2026-08-09 完成模块化单体、薄路由、组合根、遗留物理删除、当前文档与黄金资产的单实例离线关闭验收，可以进入阶段 2 的 L3 设计与统一任务语义改造。公开 check-task/Progress 与各业务契约均已按批准口径落地。Weaponry production attestation、真实生产部署、共享数据库、可靠队列、多实例、容量和 exactly-once 仍未完成，必须在对应后续阶段独立验收 |
...
| 1H | 共享文档处理与 Translator 解耦 | **已完成**：独立 `document_processing` 与 `translation` 模块拥有 MHTML/MinerU/OCR/Office、Artifact/Profile/Lineage 和语言转换；旧 Translator 包及文件处理 Facade 已由 1G 删除。阶段 2 纳入 Attempt/lease/fencing，阶段 3 再切共享 MySQL UoW/MinIO |
