...
| 接口影响 | 已确认定向调整：report 活动任务 409；三类受理成功 202 空响应体；check-task 成功 200 空响应体；check-task/Progress 严格 `params` 元素校验；Progress 显式 action 返回错误消息、保持连接且无 ack；reportId 入站接受 JSON 整数或十进制整数字符串、不设业务数值范围并按整数值规范化，输出仍为 JSON number。check-task 未来直接异步化后，200 表示恢复命令已在 MySQL 事务中可靠登记，而不表示回调已在请求返回前完成；不增删参数，接口文档措辞仍须在生产路由切换前单独确认 |
