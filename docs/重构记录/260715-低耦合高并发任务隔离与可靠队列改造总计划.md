...
17. `/llm/reassign` 上游不设置业务级最长等待时间，接口继续同步；后端必须设置有限的
    单次 AnythingLLM HTTP 超时和内部总执行预算，并预留探测/补偿窗口。预算耗尽返回既有
    HTTP 500 结构，具体秒数由集成压测校准，不新增公开参数。

**仍需在到期阶段确认**

1. 短请求成功率、P95/P99、最大排队时间、稳态压测时长和资源规格。
2. AnythingLLM 的部署拓扑、速率限制、workspace/document 幂等能力和可观测接口。
3. `/llm/reassign` 有限单次 HTTP 超时、总执行预算和补偿预留窗口的具体秒数；阶段 1E
...
6. 阶段 3～6 替换 Guard/Queue Adapter 前复核接口文档仍表达“请求内可能同步尝试回调”；
