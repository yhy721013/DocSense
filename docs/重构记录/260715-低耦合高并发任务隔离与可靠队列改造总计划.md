...
| --- | --- |
| 编写日期 | 2026-07-15 |
| 计划制订基线 | `main@0023c47`；当前阶段事实不得按该历史提交推断，应以阶段整合执行记录和关闭矩阵为准 |
| 文档状态 | 实施中；阶段 0 已按已确认决策关闭，阶段 1 已于 2026-08-09 完成模块化单体、薄路由、组合根、遗留物理删除、当前文档与黄金资产的单实例离线关闭验收。阶段 2 L3 已由 `260809-阶段2统一任务执行内核文件级实施设计.md` 重新基线化，下一步按其门禁实施。公开 check-task/Progress 与各业务契约均已按批准口径落地。Weaponry production attestation、真实生产部署、共享数据库、可靠队列、多实例和容量仍未完成，必须在对应后续阶段独立验收。系统不承诺 exactly-once；目标为至少一次投递、数据库条件写、幂等副作用和未知结果隔离 |
...
| 执行级设计 | `260715-阶段0契约容量与基础设施决策清单.md`、`260715-统一任务最小契约与Progress语义预设计.md`、`260715-阶段1A-1B文件级实施设计.md`、`260716-阶段1C报告生成文件级实施设计.md`、`260717-阶段1D武器谱文件级实施设计.md`、`260718-阶段1H共享文档处理模块文件级实施设计.md` |
| 接口影响 | 已确认定向调整：report 活动任务及 callback outcome unknown 冻结返回 409；三类受理成功 202 空响应体；check-task 成功 200 空响应体；check-task/Progress 严格 `params` 元素校验；Progress 显式 action 返回错误消息、保持连接且无 ack；reportId 入站接受 JSON 整数或十进制整数字符串，数字部分最多 128 位（正负号不计入、前导零计入）并按整数值规范化，输出仍为 JSON number；report 的 `templateDesc`/`requirement` 沿用旧链路的字符串化兼容行为；report 顶层非对象 JSON、任一非对象 params 元素及 filePathList 非字符串/空白元素在受理前按已确认文本返回 400；reassign 的外部 `false`、目标创建失败和本地条件更新失败按既有 500 结构处理。check-task 继续在请求返回前同步尝试必要恢复；未来可靠队列仅增加后台兜底，两条触发路径共用 latest-wins、lease/fencing 和 Callback Guard。不增删参数，不改变公开响应结构 |
| 数据迁移 | SQLite 历史数据仅用于测试，不迁移：阶段 2 使用独立 `task-control-v2.sqlite3` 新空库并只读预检/保留旧文件集，阶段 3 再初始化共享 MySQL Schema、测试数据和唯一写路径 |

本文把既有低耦合计划、文件对话阶段计划、其余业务结构统一，以及关于 FastAPI、并发、任务隔离和可靠队列的讨论统一为一条可执行路线。项目正式采用“模块化单体 + 按业务聚合 + 模块内分层 + Ports/Adapters”的目标结构。若本文与接口文档在公开行为上存在冲突，**始终以 `docs/接口文档/` 为准，并立即停止实现、发起确认**。2026-07-17 已确认：甲方规定 `/llm/check-task` 必须保留请求内同步报告回调恢复副作用，因此该决定取代 2026-07-16 的“以可靠队列替换同步入口”方案。未来 MySQL Outbox、RabbitMQ 和 callback Worker 作为后台可靠兜底，不改变同步返回时点；同步入口和后台 Worker 必须调用同一恢复应用服务并竞争同一 Callback Guard，禁止形成两套发送权。上述决定不授权增删请求参数、改变既有输出字段类型、错误字段、Callback 载荷或 chat 契约。
...
- **check-task 的同步恢复是永久契约，不是过渡实现。** 报告类型已在阶段 1C 通过 `RecoverReportCallbackSynchronously` 共用正常 Worker 的 latest-wins、lease/fencing 与 Callback Guard；阶段 4～6 只增加后台可靠恢复来源。不得为了并发另建无 Guard 的同步线程池，也不得让队列 Worker 绕过同一发送权。
...
3. **任务隔离**
   ...
   - Worker 崩溃或超时后，任务先进入恢复分类；只有未开始副作用或可以精确对账的任务才创建新 Attempt。旧 Worker 的数据库回写由 fencing 拒绝，外部结果未知时保守隔离。
...
| 实体/表 | 核心内容 | 目的 |
| --- | --- | --- |
| `tasks` | 内部任务 ID、类型、Task 状态、业务键、版本、当前 attempt、终态摘要 | 统一任务事实和状态机；阶段 2 SQLite 复用现有 `llm_task_executions`，不建立双写主表 |
| `task_inputs` | 不可变决策快照、源身份、配置版本和内容引用 | 阶段 2 继续内嵌于 execution 行；阶段 3 空 MySQL 再按容量和查询路径决定是否拆表 |
| `task_attempts` | 整任务执行权、lease owner、到期时间、fencing token、心跳、错误分类 | 崩溃恢复与旧 Worker 防覆盖；不混入模型或 Callback attempt |
| `task_steps` | 步骤当前投影、幂等键、副作用类别、检查点和外部引用 | 细粒度恢复和补偿 |
| `task_step_attempts` | task attempt、step attempt、意图、结果和错误分类 | 保留步骤重试历史，避免覆盖未知副作用现场 |
| `task_recovery_cases` | recovery generation、source Attempt、恢复租约/fencing、当前 Decision | 把 recovery_required 从普通执行隔离，并保证同一现场只有一个恢复 owner |
| `task_recovery_operations` | 探测/补偿 Intent、稳定幂等键、原 fencing、状态和结果时间 | 先提交 Intent 再做事务外 I/O，使崩溃接管可识别未收敛动作并防止重复补偿 |
| `task_recovery_observations` | operation 引用、结论、外部引用摘要、证据来源 | 追加保存对账证据，不用日志或人工口头结论授权重跑 |
| `task_recovery_decisions` | expected Authority、policy version、证据摘要、操作者/原因、retry checkpoint | 审计化决定继续隔离、从检查点收敛、回到 accepted 或 stale |
| `task_events` | 单调序号、事件类型、身份、原因码和脱敏元数据 | 内部审计和诊断；阶段 2 不作为事件溯源或公开 SSE 载荷 |
...
统一模型必须拆分 Task、Attempt 和 Step，不能把消息状态、执行权和外部结果画成一条状态机：
...
`queued` 属于阶段 4～5 的派发/消息事实，不在阶段 2 的 SQLite Task 状态中伪造；`leased` 属于 Attempt；`outcome_unknown` 首先属于 Step 或 Callback Delivery。外部接口继续返回当前接口文档规定的状态值。每个业务由独立 `TaskStatusPresenter` 完成映射，禁止直接把内部枚举序列化给前端。当前异步任务没有已批准的通用取消合同，因此阶段 2 不新增 `cancelled` 或公开取消入口。
...
2. 阶段 2 在同一 SQLite 事务内写入任务、不可变输入快照和内部 Task Event；阶段 4 才在同一 MySQL Unit of Work 中增加 Outbox 事件。
...
- Worker 领取任务时通过条件更新创建新的 Task Attempt，取得租约和递增 fencing token。
- 长任务定期续租；心跳失败时停止继续推进可停止的步骤。
- 所有关键状态、步骤、进度、产物引用和终态写入必须携带完整 Task/attempt/owner/lease/fencing Authority，并满足版本和到期条件。
- Reaper 只把租约过期任务列为候选；它必须读取 Step/Checkpoint 和业务恢复策略。只有可证明未产生副作用或已精确对账的任务才能重新进入 accepted，禁止通用 `running → accepted`。
- Task 进入 `recovery_required` 后，普通 Executor 不再扫描它。Recovery Coordinator 以独立
  recovery lease/fencing 记录 Observation 和 Decision；只有 `retry_authorized` Decision 才能把
  Task 原子改回 accepted，随后由普通 claim 创建新 Attempt。禁止直接改 running、复用旧 Attempt、
  覆盖旧 Step Attempt 或使用 `/llm/check-task` 重跑业务。
- 旧 Worker 即使恢复，也不能用旧 token 覆盖新 Worker 的状态和产物引用。
- 对不可安全重试的外部操作，将 Step 标记为 `outcome_unknown`，Task 进入 `recovery_required` 或沿用业务既有失败投影，由结果探测或人工补偿处理。
...
- **L2 专项/波次计划**：定义某一业务域或阶段内的垂直切片、先后顺序和完成定义，例如阶段 1 的 1A～1H。
...
| 阶段 1C～1H | 每个业务垂直切片各自的文件/端口/失败补偿设计；1C 见 `260716-阶段1C报告生成文件级实施设计.md`，1D 见 `260717-阶段1D武器谱文件级实施设计.md`，1E 见 `260724-阶段1E分类节点变更同步Saga文件级实施设计.md`，1H 见 `260718-阶段1H共享文档处理模块文件级实施设计.md`；其余最晚在对应波次开工前补齐 |
| 阶段 2～4 | 阶段 2 统一任务状态机与隔离 L3 已形成：`260809-阶段2统一任务执行内核文件级实施设计.md`；阶段 3A/3B MySQL/Artifact 与阶段 4 Outbox/Relay 仍须在各自编码前补齐独立 L3 |
...
采用模块化单体结构完成其余业务分层统一，让 Flask 蓝图成为薄适配层，并为本地执行器、Celery Worker 与 FastAPI 共享同一业务内核。波次范围以 `260715-其余业务分层统一改造实施计划.md` 为准；1A～1B 文件级步骤见 `260715-阶段1A-1B文件级实施设计.md`，1C 见 `260716-阶段1C报告生成文件级实施设计.md`，1D 见 `260717-阶段1D武器谱文件级实施设计.md`，1H 见 `260718-阶段1H共享文档处理模块文件级实施设计.md`。
...
| 1B | `/llm/check-task` 内部边界、`/llm/progress` 生产切换 | **已完成**：1B-1 固定可靠恢复命令 DTO/批量 Port/Application/Presenter/Fake；1B-2 已将 Progress 切为无 action 协议、类型化应用服务、单写入连接缓冲和线程安全单实例 Adapter。1B 的可靠命令资产后续用于后台兜底，不再替换甲方规定的同步 check-task 副作用 |
| 1C | `/llm/generate-report` | **已完成并由 1C-7 关闭验收，2026-07-17 审查补强**：除 `SubmitReportTask`、`RunReportTask(task_id)`、SQLite 持久积压和完整 Report Ports/Adapters 外，报告类型 check-task 已通过共享 Callback Guard 同步恢复；过期 Guard 有界扫描、严格 2xx、操作级清理重试水位、流式下载上限/总时限和 Dispatcher readiness 已补齐。尚未部署生产，可靠队列/多实例仍属后续阶段 |
| 1D | `/llm/weaponry` | **已完成并由 1D-7 关闭代码与 SQLite 单实例离线门禁**：四层骨架、Schema v2、Evidence Selection、任务级供应商 Adapter、Submit/Run、持久扫描 Dispatcher、Callback Guard、同步 check-task 恢复、资源恢复、组合根和公开薄路由均已落地。真实供应商 production attestation 仍是独立启用门禁，不属于阶段 1 代码关闭证明；可靠队列、多实例和生产容量继续留给后续阶段 |
| 1E | `/llm/reassign` | **已完成**：保持同步公开契约，建立可补偿 Saga、Operation/Step/Event、SQLite UoW、恢复观测和供应商 Port；共享数据库一致性与多实例演练留给后续阶段 |
| 1F | `/llm/analysis` 二次收口 | **已完成**：公开 202 空响应体，建立批量原子受理、Submit/Run、任务级 RAG/知识/翻译/审计、资源与 Callback 恢复；旧 Analysis 执行器已由 1G 物理删除 |
| 1H | 共享文档处理与 Translator 解耦 | **已完成**：独立 `document_processing` 与 `translation` 模块拥有 MHTML/MinerU/OCR/Office、Artifact/Profile/Lineage 和语言转换；旧 Translator 包及文件处理 Facade 已由 1G 删除。阶段 2 由父 Task 关联处理 Step/ExternalOperation 检查点，不复制其领域表；阶段 3 再切共享 MySQL UoW/MinIO |
| 1G | Debug、组合根和遗留清理 | **已完成**：Debug Query/Port/Adapter/Presenter、框架无关组合根和薄蓝图已收口；11/11 删除候选经运行、测试、配置、当前文档和动态导入门禁确认后物理删除，模块所有权与阶段 2 输入矩阵已冻结 |
...
- 引入 `TaskDispatcher`、Task Read、Callback Recovery、Progress、Callback、File、Artifact、Translation 等端口。check-task 同步恢复必须经业务应用服务和共享 Guard，禁止路由直接调用遗留 HTTP 发送函数。
- 将 MHTML/MinerU/OCR/Office 转换与 Artifact 谱系从 Translator/utils 抽入独立
  DocumentProcessing 模块；Translator 只单向调用其 Application 后执行语言转换。
- 阶段 1 的 DocumentProcessing Processing Record 复用 `llm_tasks.sqlite3` 的物理部署边界，但
  独立拥有表、Repository 和事务，不导入遗留 `TaskService`、不对遗留任务表建立外键。阶段 2
  由父 Task Step 保存处理记录引用并据其 checkpoint 分类恢复，不把 DocumentProcessing Attempt
  合并为整任务 Attempt；阶段 3 映射到共享 MySQL UoW 并把 Artifact 内容迁至
  MinIO，阶段 4 将异步处理派发意图与步骤事实通过 Outbox 同事务提交。
...
- 已迁移蓝图只保留协议处理、同步恢复用例调用和响应呈现，不创建后台线程；同步副作用不等于允许 Web 层持有 Guard、Repository 或 HTTP 细节。
- analysis、report、weaponry、reassign、tasks、chat 和 document_processing 的模块边界、数据所有权和依赖方向明确。
- report、weaponry、analysis 的执行入口只接收持久化任务 ID，可由兼容本地执行器和未来 Celery 共同调用。
- 报告类型 `/llm/check-task` 已接入同步恢复应用服务并与报告 Worker 共用 Guard；file/weaponry 在对应业务波次按同一原则收口。可靠恢复命令边界保留给未来后台兜底；`/llm/progress` 通过 Progress Port 工作；`/llm/reassign` 继续保持同步。
...
阶段 2 的当前权威 L3 为 `260809-阶段2统一任务执行内核文件级实施设计.md`。本节只保留总路线；状态机、SQLite 物理表策略、Authority、事务矩阵、文件范围、业务切换顺序、故障恢复和关闭门禁均以该 L3 为准。
...
- 建立分离的 Task/Attempt/Step/Event 模型：Task 表达业务执行事实，Attempt 表达 Worker 执行权，Step 表达副作用检查点；模型/Callback/领域 Step Attempt 不合并。
- SQLite 阶段复用 `llm_task_executions` 作为 Task 主事实和 `llm_tasks` 作为 latest 公开投影，新增
  Attempt/Step/Event 以及 Recovery Case/Observation/Decision，不建立第二套 tasks/task_inputs 双写表。
- 阶段 2 不原地修改旧 SQLite CHECK，也不迁移历史 Task 行；使用
  `${DOCSENSE_RUNTIME_DIR}/db/task-control-v2.sqlite3` 新空库，启动前只读预检旧文件集，并以数据库级
  application ID、Schema generation/fingerprint 和 DB instance 身份 fail closed。旧 `.sqlite3/-wal/-shm`
  原样保留，应用不得自动删除、移动或静默修表。
- 为所有任务建立版本化决策快照、源身份快照、任务目录、内容摘要/产物引用和结构化日志上下文；跨实例不可变内容 Artifact 在阶段 3B 落地。
- 移除全局可变进度回调和不可控共享客户端；按线程/任务安全范围创建依赖。
- 将阶段 1 中分析、报告、武器谱 Dispatcher 收敛为“一个共享 `LocalTaskExecutor` 内核、三个业务
  实例”：每个实例拥有本业务扫描/Runner/worker slots/readiness/未来队列路由，复用同一 claim、
  heartbeat、fencing 和停机实现；业务内 FIFO，跨业务由共享公平容量仲裁保证无饥饿而不承诺严格
  全局 FIFO。AnythingLLM ingestion、模型、文档处理和 Callback 分层限流，阶段 2 初始真实重型
  总并发仍为 1。待处理事实保存在数据库，内存只保存常量/有界唤醒，不设置业务积压数量上限。
  check-task 继续在请求线程内调用同步恢复用例，但不得创建本地积压队列或为批量项无界并行；
  所有实际发送仍由共享 Guard 串行授权。
- 建立完整 Authority 条件写、heartbeat、fencing、幂等键、终态不可逆和同业务键并发规则。
- 建立 `ClockPort`、UTC RFC3339 微秒持久时间、进程启动世代 owner 和完整租约不等式；测试使用
  FakeClock，不依赖真实 sleep。时钟异常暂停 claim/Reaper 并降级 readiness，阶段 3 再改数据库权威时间。
- Reaper 只对有 Step/Checkpoint 证据且业务策略判定安全的过期 Attempt 重试；结果未知和所有权未知保守隔离。
- `recovery_required` 从普通 runnable 集合隔离；恢复协调器以独立 recovery lease/fencing 收集
  Observation 并提交持久 Decision。只有确认未产生副作用、幂等重放已证明安全或补偿完成并
  复核通过，才原子回到 accepted；之后由普通 claim 创建新 Attempt，禁止直接改 running、复用
  旧 Attempt 或让 `/llm/check-task` 触发业务重跑。`recovery_generation` 只在进入全新的独立恢复
  现场并创建 Case 时递增，Case 领取/关闭和 `retry_authorized` 均保持不变；新执行/新恢复 owner
  分别由 `attempt_no`/Task fencing 与 Recovery fencing 表达。
- 每个业务建立版本化 Step/Recovery Registry，分别冻结 `effect_kind`、`replay_policy`、幂等键、
  Intent/Checkpoint/ExternalRef Schema、探测/补偿和 `retry_from_step`；Input Profile 使用确定性
  Canonical JSON 与 SHA-256 fingerprint，禁止从重启后的当前环境补默认值。
- 核心业务结果 unknown 时保持既有活动投影并复用同键活动冲突；终态后仅 cleanup unknown 进入
  独立维护状态且不阻塞；Callback unknown 沿用现有 Guard/check-task；不同业务键不全局阻塞。
  任何无法保持现有公开状态/409 语义的例外都必须先确认接口文档。
- Analysis、Report、Weaponry 接入统一内核；ChatRun、Reassign Saga 和 DocumentProcessing Step 保持各自唯一权威，只冻结后续映射，禁止双写。
- 增加“目录与事实所有权迁移线”：新 Task Control Store 直接拥有 SQL，旧入口只可短期单向委托；
  Task/Callback Control/Progress 与阶段 2 触达的 Report、Analysis、Weaponry 资源和审计职责从
  `app/services` 迁回明确模块/基础设施 Adapter，禁止在 `LLMTaskService` 外只套新接口；
- 2026-08-12 确认迁移顺序：阶段 2-2 只关闭新 v2 Repository/UoW/CAS，不切生产入口；阶段 2-3
  先建立完整 Authority Runtime；阶段 2-4～2-6 再按业务同时切换 Task 受理/执行/终态和 Callback
  Control。禁止只把 accepted 写入新库却让旧 Dispatcher 执行，也禁止对同一业务双写新旧库；
- SQLite 事务采用共享 Connection Factory/Transaction Manager 加 Admission/Execution/Recovery 等窄
  UoW：同一用例的 Store 共用连接、Store 不自行提交、显式 commit/默认 rollback、禁止嵌套与事务内
  网络 I/O；Admission、claim、Step intent + 业务 Resource intent、终态和 Recovery 原子组分别冻结。
- 将 `knowledge_workspace.py` 迁入小型 `app/shared/domain`；Tasks Progress 规则/Hub 归入 Tasks；
  保留 `app/adapters/web` 作为目标协议边界，不在阶段 2 整体搬迁；
- 新增 Task 配置以及 Report、Analysis 配置归入各自 `adapters/runtime_config.py`；Weaponry 现有
  模块配置在其波次等价规范命名。配置对象从环境/mapping 装载，`.env` 只是本地来源，
  `.env.example` 记录非秘密键和约束；Chat/Reassign 等未触达运行语义按后续阶段迁移；
- 保留 `${DOCSENSE_RUNTIME_DIR}/callback/` 作为 best-effort 本地诊断副本并迁移 Writer 所有权；
  Debug 继续只读，但该 JSON 不得参与 Callback 成功、重试、check-task 或 Task Recovery 判断；
- `DatabaseService`/Knowledge 表在阶段 2 只冻结 Port、写入者和阶段 3A 输入，禁止新增直接依赖；
  不先做一次 SQLite 路径搬迁、再在阶段 3 做第二次 MySQL Repository 搬迁；
- 所有结构迁移按“等价移动与导入调整 -> 定向/合同门禁 -> 新位置语义改造 -> 删除旧入口”执行，
  不在一个不可审查补丁中同时搬文件、改 SQL、改状态机和删除回滚点。
- 当前没有已批准的通用取消合同，阶段 2 不新增异步任务取消状态或公开入口；超时只产生租约过期候选，不自动等同业务失败或重试。

**验收**

- 50 个本地 Fake 重任务可同时处于持久化在途状态，输入、进度、文件和终态互不串扰；实际执行
  受 worker slots/共享容量限制，不把该测试表述为 50 个重任务并行。
- 重复派发同一任务不会重复推进已完成步骤。
- 旧 Attempt 的进度、步骤、产物引用和终态写全部被 fencing 条件拒绝。
- 同一 Recovery Case 并发处置只有一个 recovery fencing owner；`retry_authorized`、recovery
  generation（保持当前值）、retry checkpoint、Task accepted 和 Event 原子提交，新 Attempt 使用冻结输入/Profile，
  公开进度不倒退。
- Analysis 在多个本地 Worker 下仍保持同一请求内严格顺序。
- 任何任务异常不影响其他任务，且日志可按任务完整追踪。
- 50 个任务可同时处于持久化在途状态，实际重任务并发受限后仍按稳定顺序持续排空；
  Web 进程内存不随积压数量线性增长。
- 三个业务 Executor 复用同一内核、只扫描自身 task type；混合积压下共享容量无饥饿，单业务故障
  不使其他业务误领取，Control DB/Clock 异常使所有新 claim fail closed。
- 新 v2 SQLite 路径/数据库身份/旧库预检/文件集/回滚、窄 UoW、Clock/owner/租约不等式、三业务
  Step Registry 和 recovery admission 矩阵均通过定向测试。
- Report/Analysis/Weaponry 生产路径不再导入 `LLMTaskService` 或 `app.services.utils`；新 Store 不反向
  调用旧 Service，不存在双 Task/Callback SQL、双 Progress Hub 或循环委托；每张 SQLite 表只有
  一个权威写入者。
- Tasks、Report、Analysis、Weaponry 配置归属及 `app/shared/domain` 迁移完成；Callback 历史仍可
  由 Debug 读取但不参与恢复；Knowledge DatabaseService 的阶段 3A 输入和保留理由已落档。
- 接口文档、公开黄金资产与 `run.py` 零差异；安全全仓回归 0 failure、0 error，并如实注明
  SQLite 单实例/Fake 证据不能证明可靠队列、多实例或生产容量。
...
阶段 3 内部分成两个独立门禁：**3A MySQL/Alembic/Repository** 与 **3B MinIO/Artifact Catalog**。两者可以按依赖滚动实施和分别定位故障，不能用“数据库已共享”替代 Artifact 一致性，也不能用“对象已上传”替代数据库事务正确性。阶段 4 的 Outbox 设计可在 3A 稳定后开展，但任一业务切到外部 Worker 前必须同时满足 3A 和该业务所需的 3B 门禁。

**主要工作**

阶段 3 采用以下可独立关闭的交付门禁，禁止把全部数据库和对象存储工作合成一次大切换：

1. **3A-1 MySQL/Alembic 基线**
   - 冻结 MySQL 版本、字符集、数据库权威时间、事务隔离级别、连接池、锁等待/死锁处理和
     Unit of Work 规则；使用 Alembic 管理所有生产 Schema 变更，禁止应用启动时隐式改表。
   ...
   - 为高竞争领取使用短事务、条件更新或 `SELECT ... FOR UPDATE SKIP LOCKED`；不得在事务内
     执行 AnythingLLM、模型、MinIO 或 Callback 网络调用。
2. **3A-2 模块 Repository 逐一切换**
   - 按 Task/Attempt/Step/Event、Callback/Delivery、Chat、Knowledge/Resource 顺序迁移
     Schema、索引、唯一约束和外键策略；每个模块的 SQLite 兼容 Adapter 与 MySQL Adapter
     必须通过同一 Repository 契约、并发唯一性和事务回滚测试。
   - 同步 check-task 与后台 Worker 必须共享 MySQL 中同一 Callback Guard、Delivery 和发送权；
     SQLAlchemy 类型不得泄漏到 Domain/Application。
3. **3A-3 唯一写路径与恢复演练**
   - 使用空 MySQL、版本化测试种子和逐模块切换；不开发无业务价值的 SQLite 历史行迁移工具，
     也不保留长期 SQLite/MySQL 双写。
   - 完成数据校验、Alembic 升降级、备份恢复和仅在没有需保留新写入时允许的回切演练。
4. **3B-1 Artifact Catalog 与状态机**
   - 冻结输入、scratch、中间结果、最终产物的对象 key、size、系统计算的 SHA-256、所有权、
     引用、retention class 和不可变规则；内容变化必须创建新 Artifact，禁止覆盖既有可用对象。
   - Artifact 至少区分 `staging`、`available`、`orphan_candidate`、`reconcile_required` 和
     `deleting`；MinIO/S3 ETag，特别是分片上传 ETag，不得直接当作 SHA-256。
5. **3B-2 MinIO Adapter 与 staged upload**
   - 先在数据库登记 upload intent/`staging` Artifact，再上传到 staging key，核验 size 与
     SHA-256，最后在短事务中绑定业务引用并标记 `available`。对象写入和 MySQL 提交不冒充
     原子事务，任一中间失败都保留可对账事实。
   - 把阶段 1D weaponry 的文档身份/引用升级为 execution-owned 内容 Artifact；为阶段 1H
     DocumentProcessing Artifact/Profile/Lineage 实现同步 SQLAlchemy Repository 与 MinIO
     ArtifactStore Adapter。跨实例 Worker 不得依赖可变单机路径或按文件名重新选文。
6. **3B-3 对账、孤儿对象与保守 GC**
   - Reconciler 处理对象写成功但数据库提交失败、数据库引用存在但对象缺失、校验不一致和
     长期 staging 等现场；孤儿对象须经过宽限期和重新对账后才具备删除候选资格。
   - GC 领取带 lease/fencing 的删除权；删除前重新复核业务引用、任务/Attempt、恢复状态、
     Outbox、Callback、Lineage 和其他租约。删除及引用状态更新必须可审计、可恢复。
   - 未确认保留期前不启用自动扫描、MinIO TTL 或自动 GC，测试环境中的自动删除也不得被
     当作生产删除策略。

测试 Compose 必须按阶段使用 profile：3A 只启动 MySQL；3B 启动 MySQL + MinIO；阶段 5 再加入
RabbitMQ；阶段 7 再加入 Redis。应用始终从 `/venv` 运行，不进入容器且不启动 `run.py`。提前启动
某个容器只能证明依赖可达，不能证明对应队列、通知或多实例能力已实现。
...
- staged upload 在“对象上传前后、校验前后、数据库绑定前后”强制中断后均能被 Reconciler
  识别；不得留下数据库宣称可用但对象缺失的静默坏引用。
- hash 不匹配、重复上传、分片 ETag、数据库恢复后对象缺失、对象存在但数据库引用缺失均有
  明确隔离/对账结果；自动 GC 保持关闭，直至删除策略单独获批。
...
- 阶段 4 必须建立三种相互独立的派发事实：① Task 初次受理后的执行派发；② Reaper 完成
  恢复分类并明确允许新 Attempt 后的恢复派发；③业务终态提交后的 Callback 后台恢复派发。
  三类命令分别建模和路由，不能用一个含义模糊的“任务事件”代替。
- Task 受理/输入快照与初次执行 Outbox 必须在同一业务事务提交；Reaper 的恢复决定、新
  generation/Attempt 与恢复 Outbox 必须在同一事务提交；任务终态、callback delivery 待处理
  事实与 Callback Outbox 必须在同一事务提交。
- check-task 请求同步读取 latest 公共投影并尝试竞争同一 Guard；若后台 Worker 已持有发送权则返回内部 busy 结果并保持现有公开响应，不重复发出 HTTP。显式恢复 attempt 与后台 delivery attempt 均保存触发来源和 expected TaskId。
- 实现可多实例运行的 Outbox Relay，使用安全抢占、发布确认、重试和告警。
- 每条命令冻结稳定 `command_id`、聚合对象/Task ID、`dispatch_generation`、`schema_version`、
  路由类型、可投递时间、触发来源、追踪字段、负载摘要/校验值、尝试次数和状态；消息不得携带
  密钥、文件正文、模型正文、Callback 正文或公开响应对象。
- 阶段 2 的 `task_events` 只承担内部追加审计，不是 Outbox 消息表：两者的受众、保留周期、
  Payload 和重放副作用不同。禁止通过重放审计事件触发业务执行。
...
- 覆盖 Broker 已确认但 Outbox 尚未标记、毒消息、未知 schema、同一 `command_id` 重复送达和
  Relay 失权；允许重复投递，但不允许静默丢失。
...
- 实现 late ack、错误分类和退避重试；租约、fencing、Checkpoint 和 Reaper 必须复用阶段 2 已冻结语义，并在 MySQL/多 Worker 下验证，禁止另建 Celery 专用状态机。
- 冻结 Broker Delivery/Redelivery、Celery retry、Task Attempt、Step Attempt 和 Recovery Dispatch
  的决策矩阵。一次消息重投递或 Celery retry 绝不天然等于创建新 Task Attempt：Consumer 必须
  先查询 MySQL Authority，并按 `command_id`、generation、当前 Attempt、租约与 fencing 条件
  作出执行、ACK、等待或隔离决定。
- Consumer 发现 Task 已终态或命令已过期时直接 ACK 且不执行；发现合法 Attempt 正由其他
  Worker 持有时不得重复推进；只有持久恢复事务判定 `retry_safe` 并创建新 Attempt/Outbox 后，
  才允许新一轮业务执行。租约过期但外部结果未知时进入 `recovery_required`，禁止热重排队。
- Celery Result Backend 不作为业务真相源；自动 retry 不得无条件包裹 AnythingLLM、模型、
  MinIO、Callback 等外部副作用。MySQL 暂时不可用且无法完成 Authority 判断时不 ACK，由传输层
  重投递，但不得私自推进业务状态。
- 建立发布、消费、执行、回写、确认的全链路 trace。
- 通过配置开关按任务类型从本地 Dispatcher 切到队列 Dispatcher；check-task 同步入口不随任务 Dispatcher 切换而删除，组合根只替换其 Guard/Recovery Source 的基础设施 Adapter。
- 对 RabbitMQ 不可用、Worker 退出、网络分区、消息重复和数据库暂时不可用进行故障注入。

**验收**

- Web 进程在任务受理后立即退出，任务仍可被其他 Worker 完成。
- Worker 在执行中被终止后，租约到期只产生恢复候选：未开始副作用、具有稳定幂等结果或可以
  精确对账的任务才创建新 Attempt；AnythingLLM、模型或 Callback 结果未知时必须保留
  `recovery_required`/领域 unknown 事实，不能统一宣称自动安全接管。
- 同一消息重复投递不会产生两个成功终态或覆盖新 attempt。
- 队列不可用时任务保留在 Outbox，恢复后自动补发。
- 强杀矩阵覆盖“外部意图登记前、登记后发送前、发送后响应前、响应后数据库提交前、终态提交后
  ACK 前”；旧 fencing 只能阻止数据库写入，不能被描述为能够撤回已经发出的网络请求。
...
1. **先切 callback 后台可靠兜底，不删除 check-task 同步路径**：终态事务通过 Outbox 派发 callback Worker；Worker 与 Web 同步恢复用例都在同业务键 Guard 内执行 latest-wins、外发和条件持久化。同步路径不是队列故障回退开关，而是甲方长期契约；二者并存但同一时刻只能有一个 lease owner。
2. 清理/补偿任务：设计为可重复执行，验证死信和人工重放。
3. 报告生成：先切换阶段 2 试点 `RunReportTask(task_id)`，验证长耗时步骤、检查点和产物提交。
4. 分析任务：切换 `RunAnalysisTask(task_id)`，重点验证批次顺序、受理、进度、结果和失败映射。
5. 武器谱任务：切换 `RunWeaponryTask(task_id)`，验证检索限流、输入快照和回调。
6. 文件对话重步骤：在现有阶段 1～12 基础上进入后续 Worker 化。

**特殊边界**

- 派发切换不能只依赖每个 Web/Worker 实例自己的环境变量。MySQL 保存按任务类型划分、可审计、
  带 `policy_version`/生效 generation 的全局切换状态：
  `local_only -> shadow_publish -> queue_active -> local_drain -> local_removed`。
  `shadow_publish` 只验证信封、路由、指标和去重，影子 Consumer 没有副作用执行权；
  `queue_active` 后新任务只走队列；`local_drain` 期间本地执行器只完成切换前已持有任务，不再领取
  新任务；确认旧在途任务、旧 Outbox 和回滚窗口清空后才进入 `local_removed`。
- 每个 Task 在受理事务中冻结 dispatch path、policy version 和 generation。多实例滚动发布时，旧
  实例读取到过期策略必须 fail closed，不得按本地默认值领取新任务。回滚只改变新受理任务的
  派发方式；已经进入新队列的任务继续由新 Consumer 排空，绝不能交回旧执行器。
- RabbitMQ 暂时不可用不阻止 check-task 依照现有契约同步尝试；终态 Outbox 事实继续保留，Relay 恢复后后台处理。后台 Worker 必须先读 Guard，不能把同步已成功的回调再次发送。
...
额外覆盖：不同版本 Web/Worker 同时在线、部分实例重启、shadow 消息重复、`queue_active` 时仍有
本地在途任务、带积压回滚和旧实例读取到过期策略。任一时刻同一 Task 只能有一条具备数据库
Authority 的执行路径。
...
- 建立“订阅—快照”屏障：先订阅并临时缓冲通知，再读取带单调版本的 MySQL 快照；发送快照后
  丢弃版本不大于快照的缓冲项，只交付更新版本，再进入实时模式。等价实现必须证明快照读取
  期间发生的通知既不丢失，也不会倒退客户端进度。
- Redis 重复、乱序通知按单调版本过滤；Redis 重连后执行有界数据库补查。同一进程、同一业务键
  聚合为一份订阅并向本地连接扇出；每连接使用有界合并缓冲，慢连接只覆盖自身旧进度。
- Progress/WebSocket 连接断开时只取消本机订阅并释放资源，不取消原异步业务任务；Chat SSE
  断线服从阶段 9 和现有对话接口合同，不能套用这一条后台任务语义。
...
- 覆盖通知发生在订阅前、快照读取中、快照发送后，以及重复、乱序、Redis 重连和慢连接场景；
  现有 `/llm/progress` 不新增 cursor、ack、事件 ID 或公开内部版本字段。
...
- 阶段 9 的 ChatRun 多实例 lease/fencing 关闭前，普通 HTTP 和 Progress 可以逐步多实例灰度，
  但 `/llm/chat*`、`/llm/weaponry-chat*` 必须由网关或数据库全局策略路由到单一受理所有者；
  普通粘性会话不能替代正确性门禁。FastAPI Chat 可先做合同测试、影子验证或单实例受理。
- Web lifespan 只初始化 Web Adapter、数据库/Redis/HTTP Client 和连接资源，不得再次启动本地
  业务 Dispatcher 或重型执行线程；阶段 6 已切出的业务只能由独立 Worker 执行。
...
- 双栈同时在线时 Chat 对外仍只有一个合法受理者，且 Web 进程中不存在重新创建的本地业务
  Dispatcher。阶段 8 压测只证明 Web Adapter 和长连接承载，不证明 Chat 多实例执行已完成。
...
- Task 只承担可靠派发和 Worker 执行 Authority，ChatRun 继续唯一拥有消息、Chunk、流终态、
  中断和历史；两者通过内部引用和同一恢复协议协作，禁止形成两套可独立成功的终态。
- Worker 不依赖 Flask/FastAPI 请求线程执行，但现有公开合同仍规定调用方在成功终态前关闭 SSE
  后必须停止本轮。Web 观察到传输关闭时持久化中断/断线事实，Worker 通过数据库或可丢通知观察
  后收敛为失败/aborted；不保存不完整 assistant/Chunk，也不继续后台生成。
- 调用 `/llm/chat/abort` 或 `/llm/weaponry-chat/abort` 是显式业务中断；SSE 传输意外关闭是连接
  生命周期事件；Web 进程终止则由连接代理、租约和恢复分类共同处理。三者触发来源不同，但都不能
  绕过现有“断线不继续生成、无不完整 assistant”的公开语义。
- 内部事件账本可以支持当前仍存活连接从 MySQL 补取 Redis 丢失的通知、Worker/Web 解耦交付和
  故障审计；不得向重连调用方提供 SSE 回放或断线续传，不得为此新增公开 TaskId、RunId、cursor、
  Last-Event-ID 或其他参数/字段。
- 冻结成功终态与断线/中断竞态：完整 assistant/Chunk 和成功终态先提交时，随后断线不得回滚
  已提交历史；中断/断线条件写先成功时，Worker 后续成功提交必须被 CAS/fencing 拒绝。
...
- 覆盖显式 abort、SSE 在执行权领取前关闭、生成中关闭、成功事务提交竞态、Web 进程终止、
  Worker 崩溃、Redis 丢失和实例重启；所有场景均符合现有接口契约，且没有对外 SSE 回放。
...
- 分别统计不少于 50 个并发短请求、50 条 WebSocket/SSE 长连接、50 个持久化在途任务和各类
  重型 Worker 实际并行度；四个数字不能混成一个“并发 50”。重型并行度按 AnythingLLM、模型、
  CPU/GPU、内存和供应商配额独立冻结。
- 冻结可复现 workload mix：任务类型比例、到达率、文件大小、上下文/输出长度、供应商 Fake 或
  真实模式、warm-up、soak 时长、P50/P95/P99、错误率、最大排队时间、积压恢复时间和 Callback
  最终完成时间。是否提高短请求到 100 由首次完整集成环境资源基线决定。
- “取消”只覆盖现有 Chat/Weaponry Chat abort 与断线合同；不得为 Analysis、Report、Weaponry
  虚构通用取消接口、参数或状态。
...
- 单物理服务器上的多进程实验只证明进程隔离、重启恢复和共享状态竞争，不得据此宣称主机级
  高可用、跨主机网络分区容错或存储节点故障切换；这些结论需要额外主机和独立故障域证据。
...
- 所有生产 Schema 使用 expand/contract：先新增兼容字段/索引，再完成 N/N-1 应用双读或兼容
  读取、数据回填和新写路径切换；只有旧实例、旧任务和回滚窗口清空后才停止旧写并执行破坏性
  删除。消息 schema 同样至少兼容当前与上一版本，未知版本进入隔离/DLQ 而不是错误执行。
- 达到稳定观察周期后，依次删除 daemon thread、本地 Dispatcher、Flask 入口和 SQLite 生产写路径。
- 删除前除静态搜索、运行指标和部署配置外，还必须证明旧任务、旧 Outbox、旧队列消息、DLQ
  待处理项及回滚窗口均已清空；已经进入新队列的任务不能回交旧执行器。
- 更新运行、部署和故障处理文档；若需要修改接口文档，必须先单独确认。
- 生产语义明确写为“至少一次投递 + 条件写 + 幂等副作用 + 未知结果隔离”；系统不承诺
  exactly-once，也不把没有重复终态误写为所有外部副作用恰好执行一次。
...
| 任务状态检查 | 参数集合、业务键语义、已批准目标成功 200 空响应体、非对象 params 整次 400、其他 400/404 错误；目标 200 表示本次同步检查/必要恢复流程已完成，不公开内部结果。当前 legacy JSON/过滤行为仍按接口文档实施状态保留，须由独立波次切换 | 内部统一状态泄漏到响应、非法元素被部分处理、并发请求放大回调、后台 Worker 绕过同步入口的 Guard，或误让 RabbitMQ 可用性决定同步入口能否返回 |
...
7. 甲方规定 check-task 保留请求内同步回调恢复；报告类型已在阶段 1C 使用共享 Guard 实现。
   MySQL Outbox + RabbitMQ + callback Worker 是后台可靠兜底，不替换同步入口；两条路径
   必须共用 expected TaskId、lease/fencing 与 delivery outcome，禁止双发送。
...
16. report RAG 返回 `None`、空字符串或纯空白内容时继续生成空 HTML 并按成功回调；仅增加
    内部日志和指标，不改变公开状态、字段或交互审计硬门禁。
17. `/llm/reassign` 上游不设置业务级最长等待时间，接口继续同步；后端必须设置有限的
    单次 AnythingLLM HTTP 超时和从 Application 收到命令起计算的远端调用总预算，并预留
    探测/补偿窗口。预算耗尽返回既有 HTTP 500 结构，具体秒数由集成压测校准，不新增公开参数。
    当前 SQLite 阶段只扣减锁等待耗时，阶段 3 必须用数据库权威时间和可取消语句/锁等待补齐
    整个请求的硬 deadline。
...
3. `/llm/reassign` 有限单次 HTTP 超时、远端调用总预算和补偿预留窗口的具体秒数；阶段 1E
   使用可注入有限值完成离线验证，首次完整集成环境校准，阶段 10 前冻结生产配置。
4. 新数据库/队列 Adapter 必须通过“事务已提交但确认丢失”的终态对账故障测试，并保持领域保护
   状态、Repository 活动索引和文档删除门禁三方一致；该门禁不得因新增状态静默漂移。
5. Docker Compose 各组件的版本、资源、备份、监控、密钥管理和运维责任人。
6. MinIO 各对象类别的具体保留天数及 callback payload 删除后的 check-task 可恢复窗口；
   在启用首条非测试自动删除规则前必须确认，可能影响接口说明时须再次确认。
7. 阶段 3～6 替换 Guard/Queue Adapter 前复核接口文档仍表达“请求内可能同步尝试回调”；
   若未来甲方允许改变该时点，必须另行确认，且仍不得增删任何请求或回调参数。
...
3. 用兼容 Facade 包装当前 `LLMProgressHub`；Task Read 保留只读兼容 Facade，check-task 同步恢复通过业务 Callback Port 和共享 Guard 实现。
4. 保留 `/llm/check-task` 已完成的框架无关 Task Read/可靠命令资产；报告切片另行实现同步恢复用例，可靠命令在后续阶段作为后台兜底触发源。
...
这个切片能先验证模块边界、任务状态检查命令边界和 Progress Port，不要求启动 `run.py`，也不要求立刻具备 MySQL/RabbitMQ 环境。`/llm/check-task` 不是纯只读查询：报告类型已迁移到同步恢复应用服务；阶段 3～6 只替换共享持久化和增加后台 Worker，不改变其同步触发职责。
...
这个顺序既允许在 Flask 仍在线时逐业务完成结构收口，也避免在接入 Celery 或迁移 FastAPI 时再次拆分报告、武器谱和任务状态检查。最终系统中，analysis、report、weaponry、reassign、chat 和 tasks 是高内聚业务模块，document_processing 是独立共享能力模块；Web 框架、任务队列、数据库、AnythingLLM、文档转换器和存储都是可替换适配器；稳定的领域规则、应用用例和公开接口契约保持在架构中心。
