...
| 编写日期 | 2026-07-15 |
| 适用基线 | `main@0023c47` |
| 文档状态 | 实施中；阶段 0 已有条件关闭，阶段 1 的 1A、1B、1C 和 1D-0/0R～1D-7 开发分支代码与验收已完成。weaponry 的真实 Callback Guard、同步 check-task 恢复、资源恢复、生产组合根、202 空体薄路由、Creation Intent、HTTP 租约、永久 AST/配置门禁和遗留清单均已落地；关闭后又补强真实 Source URL 身份、共享 FIFO limiter、Schema v2 有时效证明和人工处置审计。localhost 只读 score/来源身份已验证 8/8，但四类生产指纹未冻结、完整 production attestation 未生成，readiness 必须保持 false；代码尚未部署生产。共享文档处理平级阶段 1H 已完成 1H-0～1H-7 代码与安全离线关闭验收；生产 Report/Analysis/Translation/Weaponry 已切换到单一 prepared Artifact/TranslationEngine 路径，旧 Handler/Service 按引用矩阵保留且未物理删除，有效 Artifact 的独立 GC 留待阶段 3 以后。报告与 weaponry 类型 check-task 已使用各自业务模块内共享的 Callback Guard 保留甲方要求的同步恢复副作用，阶段 3～6 增加可靠队列后台兜底但不替换同步入口；已批准的 check-task 200 空响应体尚未在本波次切换；真实性能基线按批准延期 |
...
| 1D | `/llm/weaponry` | **1D-1～1D-5 已完成；1D-6 开发分支实现与安全离线门禁完成；待真实供应商补测和 1D-7**：四层骨架、唯一 Schema v2、score/rank Selection、生产 I/O Adapter、Submit/Run/字段 Application、通用持久扫描 Dispatcher/进程锁、Weaponry 单执行 Worker、严格配置、共享 limiter、真实 Callback Guard、同步 check-task 恢复、资源有界恢复、生产组合根和公开 202 空体薄路由均已落地。50 个 accepted 只形成持久行、一条执行线程和零内存队列项；215 项 weaponry、40 项 stage1d、69 项关联回归及 1130 项安全全仓回归通过。这些仍不代表真实模型吞吐或可靠队列/多实例能力；真实 score/rank、来源身份、Provided-Evidence 模型行为与生产容量尚未验收。原始文件处理移交 1H；源码链已切换但尚未部署生产 |
...
| 1H | 共享文档处理与 Translator 解耦 | 独立 `document_processing` 模块、MHTML/MinerU/OCR/Office Adapter、不可变 Artifact/Profile/Lineage、任务级资源和 Translator 兼容 Facade；阶段 1 共用 `llm_tasks.sqlite3` 但独立持表/Repository，阶段 2 纳入 Attempt/lease/fencing，阶段 3 再切共享 MySQL UoW/MinIO |
| 1G | Debug、组合根和遗留清理 | Debug Adapter、框架无关 Bootstrap、薄蓝图和旧路径清理证据；1H-7 已输出矩阵但兼容源码/测试引用未清零，最终删除继续等待可执行引用实际归零及回滚观察期结束 |
...
- 阶段 1 的 DocumentProcessing Processing Record 复用 `llm_tasks.sqlite3` 的物理部署边界，但
  ...
  再把处理步骤纳入 Attempt/lease/fencing，阶段 3 映射到共享 MySQL UoW 并把 Artifact 内容迁至
...
- 将阶段 1 中分析、报告、武器谱等仍需兼容运行的 Dispatcher 收敛为有界、可关闭、可观测的本地执行器，并保持现有接口返回时机；待处理事实保存在数据库，内存只保存常量/有界唤醒，不设置业务积压数量上限。check-task 继续在请求线程内调用同步恢复用例，但不得创建本地积压队列或为批量项无界并行；所有实际发送仍由共享 Guard 串行授权。
...
- 为任务、知识库和文件对话梳理统一 Schema、索引、唯一约束和外键策略；同时把 `callback_delivery_guards`、`callback_deliveries`、后台恢复请求与 Outbox 迁入 MySQL，使同步 check-task 与后台 Worker 在多实例下共享同一权威发送权。
...
- 把阶段 1D weaponry 冻结的文档身份/引用升级为 execution-owned 内容 Artifact，保存对象引用、
  size、SHA-256 与所有权；跨实例 Worker 不得按文件名重新选文或依赖可变单机路径。
- 为阶段 1H 的 DocumentProcessing Artifact/Profile/Lineage 实现同步 SQLAlchemy Repository 和
  MinIO ArtifactStore Adapter；数据库是引用、所有权和删除资格真相源，MinIO 只保存对象字节。
- 为共享 Artifact 增加独立 GC 状态机：按 retention class 计算可空删除资格，领取带 fencing 的
  删除权，删除前复核任务终态、租约、重试、Outbox、回调恢复及其他引用；对象删除与引用/lineage
  状态更新必须可审计、可恢复。未确认具体保留期前不启用自动扫描或 MinIO TTL。
...
- 在任务形成终态的同一业务事务内写 callback delivery 待处理事实与 Outbox 事件，确保没有 check-task 请求时后台兜底仍可继续处理。
