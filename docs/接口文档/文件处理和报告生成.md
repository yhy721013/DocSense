...
- 服务端只要求最终 `architectureId` 是请求 `architectureList` 中的整数候选；父节点属于合法结果，不会因不是叶子节点而拒绝成功回调或永久入库。
...
- 多候选场景下，首次 `architectureId` 不合规时会先按 GJB/国军标正文线索匹配数据标准候选，再执行一次分类修复；最终仍未返回请求中的整数候选时，任务解析失败，不发送成功回调，也不写入永久知识库。
