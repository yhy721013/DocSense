...
2. `/llm/chat` 的既有字段和 SSE `chatInfo`、`textChunk`、`done` 格式属于冻结行为；其中 `chatId` 的值类型为正整数，JSON 请求严格拒绝字符串、布尔值、零和负数；内部 `run_id`、Scope Revision、租约和任务标识不得泄露。`fileNames` 的空/非空状态机和 history `files` 的取值语义不得借机扩展协议。
