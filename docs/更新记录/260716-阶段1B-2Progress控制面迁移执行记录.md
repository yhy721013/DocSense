...
6. 合法批量消息按 params 位置发送当前快照；重复业务键保留重复的当前快照位置，但只
   建立一条底层订阅，避免后续通知倍增。
7. 报告类型 `reportId` 接受 JSON 整数或十进制整数字符串，不设置 32/64 位机器整数
   范围，但数字部分最多为 128 位；正负号不计入、前导零计入该上限。同整数值按同一
   业务键订阅。格式或长度错误返回 error 后连接仍可继续处理合法消息。
...
批准 `reportId` 入站接受 JSON 整数或十进制整数字符串并按整数值规范化；全面审查后
又批准数字部分最多为 128 位。接口文档和阶段 0 黄金资产已据此更新。请求字段集合、
响应字段与 `data.reportId` 的 JSON number 输出类型仍未改变。
...
5. `/llm/check-task` 在本波次结束时继续使用旧同步恢复和旧成功 JSON；2026-07-17 最终
   决策要求永久保留同步副作用，1B-1 可靠命令改作后台兜底。report 分支随后已接入共享
   Guard，已批准空响应/严格校验目标仍应由独立契约波次切换，而非随队列一次性替换。
...
兼容与 128 位数字上限，本轮按该授权同步了接口文档；字段集合和响应类型保持不变。
