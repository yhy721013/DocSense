...
| 文件 | 作用 |
| --- | --- |
| `260803-永久知识谱系Workspace命名执行记录.md` | 阶段 0～7 已完成：Analysis 入库、Reassign 前向与恢复统一使用 `archId-{规范化architectureId}`，旧前缀生产引用清零且不兼容迁移旧数据；安全全仓发现 2,193、排除 13、执行 2,180 项，并完成本机回环 AnythingLLM 的 Analysis/Weaponry/Reassign 隔离创建、精确核名和基线恢复。 |

## Weaponry 临时资源清理

| 文件 | 作用 |
| --- | --- |
| `260802-武器谱临时Workspace即时持久清理执行记录.md` | 业务终态持久化后立即提示独立资源维护线程，逐项恢复尝试有界且按任务轮转续扫；保留定时恢复、lease/fencing、失败退避与 unknown 隔离，不等待 Callback、不按前缀盲删且公开契约零修改。 |
...
| 文件 | 作用 |
| --- | --- |
| `260801-阶段1G-0契约引用与回归基线执行记录.md` | 冻结 15 条 Flask 路由、四条 Debug 路由的内部契约和 11 组删除候选；关闭复验只读扫描 559 个 Python/307 个文本文件并形成 911 条分类事实，54 项离线门禁通过，运行代码与接口文档零修改。 |
| `260801-阶段1G-1Debug查询边界与薄路由执行记录.md` | 建立 Debug 不可变结果、Query/Port/Adapter/Presenter 和框架无关组合根，将四条 Debug 路由收口为薄入口、旧 Preview 收口为兼容 Facade；54 项计划分组与 85 项扩大门禁通过，旧 Preview 生产运行引用清零。 |
| `260801-阶段1G-2框架无关组合根与Flask依赖访问执行记录.md` | 将 `current_app.extensions` 集中到唯一 Flask 依赖 Adapter，移除生产组合根的 Web 框架依赖并新增永久门禁；4 个实际容器构造点显式装配 Debug，138 项计划门禁与 19 项交叉契约门禁通过。 |
| `260801-阶段1G-3正式蓝图关闭审查与永久门禁执行记录.md` | 将 Weaponry 文档冻结/策略注入/受理及 check-task 预校验/同步恢复/同 TaskId 重读收口到 Application，保持 Chat SSE、Progress WebSocket 与 Reassign 顺序；105 项主门禁、102 项定向契约和 87 项扩大门禁通过。 |
| `260801-阶段1G-4遗留测试价值迁移执行记录.md` | 将十个旧测试文件的 200 条断言逐方法映射到现行 Domain/Application/Adapter/Presenter 测试；旧安全网 200 项、迁移后核心语义 120 项、终检 52 项及安全全仓 2,112 项门禁通过，随后才进入负责人四项确认与 1G-5 物理删除。 |
| `260801-阶段1G-5条件物理删除执行记录.md` | 按 A1～A5、B～E 顺序删除旧业务执行器、Translator/文档处理/Debug Facade、AnythingLLM/RAG 包装及四个方法级兼容入口；11/11 候选删除就绪，安全全仓发现 2,115、排除 13、执行 2,102 项且失败/错误为 0，公开接口文档零修改。 |
| `260801-阶段1G-6关闭验收与阶段2交接执行记录.md` | 固化模块/数据库表所有权、依赖方向、保留适配、回滚点和阶段 2 输入，完成九组顺序验收；安全全仓发现 2,118、排除 13、执行 2,105 项，失败/错误 0、跳过 3，阶段 1G 正式关闭。 |
...
| `260802-文件分析RAG图片占位文本移除执行记录.md` | RAG 投影 v2 完整移除 Base64 图片语法且不再生成提示、alt、媒体类型、摘要或 payload 长度，只保留 Token 分隔空格；升级 Profile/step 并保持公开接口零修改。 |
