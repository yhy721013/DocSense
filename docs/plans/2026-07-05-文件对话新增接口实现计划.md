...
- **标题后处理**：对大模型返回的文本做 `strip()` 清理，去除引号和多余标点，最多保留 20 个中文字符，服务端强制截断。
...
def generate_chat_title(
    *,
    ...
    title = _clean_title(answer.text, max_length=20)
    return title
```
...
def llm_chat_title():
    ...
    try:
        ...
        return jsonify({"chatId": chat_id, "title": ""})
...
并发约束：同一 `chatId` 同时只允许一个活跃流式响应。`ChatAbortRegistry.register()` 应拒绝重复注册同一 `chatId`，`/llm/chat` 路由在检测到重复活跃流时返回 409，要求调用方等待上一条流收到 `done`、`aborted` 或 `error` 后再发起下一轮。
...
class ChatAlreadyStreamingError(Exception):
    """同一 chatId 已存在活跃流式响应。"""

    def __init__(self, chat_id: str):
        self.chat_id = chat_id
        super().__init__(f"当前对话已有进行中的流式响应: {chat_id}")
...
class ChatAbortRegistry:
    ...
    def register(self, chat_id: str) -> threading.Event:
        """为新流式请求注册中断 Event；同一 chat_id 已有活跃流时抛出异常。"""
        abort_event = threading.Event()
        with self._lock:
            if chat_id in self._active:
                raise ChatAlreadyStreamingError(chat_id)
...
| 层级 | 文件 | 改动 |
|------|------|------|
| 接口层 | `app/blueprints/llm.py` | 新增 `/llm/chat/abort` 路由；修改 `/llm/chat` 路由传入 `abort_registry`，同一 `chatId` 活跃流重复请求返回 409 |
| 业务层 | `app/services/llm_service/chat_service.py` | 新增 `ChatAbortRegistry`、`ChatAlreadyStreamingError`；`handle_chat_stream()` 增加 `abort_registry` 参数和中断检查逻辑 |
