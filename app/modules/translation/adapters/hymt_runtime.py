"""HYMT/Argos/Ollama 翻译引擎的唯一运行时实现。"""
...
from .hymt_support import ProgressTracker, build_prompt, clean_output


logger = logging.getLogger(__name__)


def _apply_translation_third_party_log_levels() -> None:
    """Argos 延迟导入后重新应用 Translation 自有的第三方降噪策略。"""
...
class HYMTTranslator:
    ...
    def __init__(
        ...
        :param ollama_read_timeout_seconds: 单次非流式生成的有界读取超时
        """
        del device_map
        if (
            isinstance(ollama_read_timeout_seconds, bool)
            or not isinstance(ollama_read_timeout_seconds, (int, float))
            or ollama_read_timeout_seconds <= 0
        ):
            raise ValueError("ollama_read_timeout_seconds 必须为正数")
        self._ollama_timeout = (10.0, float(ollama_read_timeout_seconds))
    ...
    def _translate_with_argos(self, text: str, target_lang: str) -> str:
        ...
        try:
            ...
            _apply_translation_third_party_log_levels()
