...
def _augment_leaf_candidates(
    ...
    *,
    ...
    base_leaf_limit: int = MAX_BASE_LEAVES,
    leaf_candidate_limit: int = MAX_LEAF_CANDIDATES,
) -> tuple[int, ...]:
    ...
    for parent_id in preferred_parent_ids:
        ...
        if not family:
            continue
        family_parent_ids.append(parent_id)
        _append_unique(selected, family, leaf_candidate_limit)
    ...
    for parent_id in family_parent_ids:
        _append_unique(selected, _equipment_family(index, parent_id), leaf_candidate_limit)

    standard_leaf_ids = _data_standard_leaves(index)
    if set(base_ids) & set(standard_leaf_ids) or set(rule_ids) & set(standard_leaf_ids):
        _append_unique(selected, standard_leaf_ids, leaf_candidate_limit)
    ...
    for root_id in sorted(hit_roots, key=lambda node_id: index.require(node_id).ordinal):
        ...
        if len(root_leaves) > base_leaf_limit:
            continue
        for child_id in index.children_by_id[root_id]:
            ...
            _append_unique(selected, (best_leaf,), leaf_candidate_limit)
            if len(selected) >= leaf_candidate_limit:
                break
        if len(selected) >= leaf_candidate_limit:
            break

    return tuple(selected[:leaf_candidate_limit])
...
def _eligible_parent_ids(
    ...
    *,
    ...
    return tuple(item[0] for item in eligible[:limit])
...
class ArchitectureRecallService:
    ...
    def __init__(
        ...
        *,
        ...
        base_leaf_limit: int = MAX_BASE_LEAVES,
        parent_candidate_limit: int = MAX_PARENT_CANDIDATES,
        model_candidate_limit: int = MAX_FINAL_CANDIDATES,
    ) -> None:
        if (
        ...
        limits = (
            ("基础叶候选上限", base_leaf_limit, MAX_BASE_LEAVES),
            ("父候选上限", parent_candidate_limit, MAX_PARENT_CANDIDATES),
            ("模型候选上限", model_candidate_limit, MAX_FINAL_CANDIDATES),
        )
        for name, value, hard_limit in limits:
            if (
                ...
                raise ValueError(f"{name}必须是 1..{hard_limit} 的整数")
        if base_leaf_limit + parent_candidate_limit > model_candidate_limit:
            raise ValueError("基础叶候选上限与父候选上限之和不得超过模型候选上限")
        self._index = index
        self._prompt_char_limit = prompt_char_limit
        self._prompt_overhead_chars = prompt_overhead_chars
        self._base_leaf_limit = base_leaf_limit
        self._parent_candidate_limit = parent_candidate_limit
        self._model_candidate_limit = model_candidate_limit
    ...
    def recall(
        ...
        *,
        ...
        base_ids = fused_ids[:self._base_leaf_limit]
        leaf_ids = _augment_leaf_candidates(
            ...
            base_leaf_limit=self._base_leaf_limit,
            leaf_candidate_limit=(
                self._model_candidate_limit - self._parent_candidate_limit
            ),
        )
        if scoped_ids is not None:
            leaf_ids = tuple(
        ...
        ordinary_parent_ids = _eligible_parent_ids(
            ...
            direct_tree_ids,
            rrf,
            limit=self._parent_candidate_limit,
        )
        parent_ids: list[int] = list(preferred_parents)[
            :self._parent_candidate_limit
        ]
        _append_unique(
            parent_ids,
            ordinary_parent_ids,
        ...
        parent_ids = parent_ids[:self._parent_candidate_limit]
        if scoped_ids is not None:
            parent_ids = [
                node_id for node_id in parent_ids if node_id in scoped_id_set
            ]
        final_ids = (*leaf_ids, *parent_ids)
        if len(final_ids) > self._model_candidate_limit:
            raise ArchitectureRecallError(
                f"领域候选数量超过策略上限 {self._model_candidate_limit}"
            )
...
def recall_architecture_candidates(
    ...
    *,
    ...
    return ArchitectureRecallService(
        ...
        base_leaf_limit=base_leaf_limit,
...
__all__ = (
    ...
    "MAX_REMARK_CHARS",
    "MAX_CLASSIFICATION_PROMPT_CHARS",
    "CLASSIFICATION_PROMPT_OVERHEAD_CHARS",
    "BM25_K1",
    "BM25_B",
    "BM25_LIMIT",
    "TREE_ROOT_BEAM",
    ...
    "_IDENTIFIER_RE",
    "_GJB_RE",
    "_JANE_UUV_TITLE_ALIAS_RE",
    "_JANE_UUV_TYPE_DESCRIPTION_RE",
    "_JANE_UUV_CANONICAL_NAME",
    "_JANE_TITLE_TYPE_ALIAS_REASON",
    ...
    "_get_bm25_corpus",
    "_bm25_rank",
    "_contains_ascii_name",
    "_direct_exact_ids",
    "_expand_exact_leaves",
    "_local_route_scores",
    "_tree_rank",
    "_detail_kind",
    "_equipment_family",
    "_data_standard_roots",
    "_data_standard_leaves",
    "_rule_rank",
    "_jane_title_type_alias_rule_ids",
    "_rank_lookup",
    "_rrf_scores",
    "_best_channel_rank",
    "_ordered_fused_leaves",
    "_append_unique",
    "_augment_leaf_candidates",
    "_eligible_parent_ids",
    "_PREFERRED_PARENT_REASON_PRIORITY",
    "_normalize_preferred_parent_reasons",
    "_projection_chars",
    "_normalize_candidate_scope_ids",
    "_normalize_candidate_remark_overrides",
