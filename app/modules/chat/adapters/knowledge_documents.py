"""从 DocSense 本地知识记录解析不可变的 Chat 文档快照。"""
...
from app.integrations.anythingllm.models import normalize_source_marker
...
class DatabaseChatDocumentResolver(ChatDocumentResolver):
    ...
    def _resolve_architecture_records(
        ...
        for index, record in enumerate(records):
            ...
            raw_original_name = record.get("original_name")
            if not isinstance(raw_original_name, str):
                raise TypeError("architecture record original_name must be str")
            original_name = raw_original_name.strip()
            if not original_name:
                raise ValueError("architecture record original_name is empty")
            metadata = record.get("metadata")
            if not isinstance(metadata, Mapping):
                raise TypeError("architecture record metadata must be mapping")
            raw_source_key = metadata.get("docSource")
            if not isinstance(raw_source_key, str):
                raise TypeError("architecture record metadata.docSource must be str")
            source_key = normalize_source_marker(raw_source_key)
            if not source_key or source_key != raw_source_key:
                raise ValueError("architecture record metadata.docSource is invalid")
            ...
            if source_key in seen_source_keys:
                raise ChatDocumentCatalogConflictError(
                    "architecture scope contains duplicate structured source key"
                )
