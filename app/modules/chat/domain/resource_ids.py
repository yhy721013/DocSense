...
def chat_workspace_lease_id(conversation_id: str) -> str:
    return f"chat:{_required_text(conversation_id, name='conversation_id')}:workspace"


def chat_thread_lease_id(conversation_id: str) -> str:
    return f"chat:{_required_text(conversation_id, name='conversation_id')}:thread"


def chat_temporary_thread_lease_id(*, conversation_id: str, attempt_id: str) -> str:
    ...
    return f"chat:{normalized_conversation_id}:temporary_thread:{normalized_attempt_id}"
...
def chat_document_binding_lease_id(
    *,
    ...
    if normalized_document_ref:
        ...
        suffix = f":{digest}"
    return f"chat:{normalized_conversation_id}:document_binding:{normalized_file_name}{suffix}"
