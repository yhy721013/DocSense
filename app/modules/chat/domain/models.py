...
from app.modules.chat.domain.document_scope import ChatRequestedFile
...
@dataclass(frozen=True)
class ChatMessageSourceChunk:
    """一条 assistant 消息按上游顺序冻结的无损来源原文。"""
    ...
    def __post_init__(self) -> None:
        ...
        original_file_name = str(self.original_file_name or "").strip()
        created_at = str(self.created_at or "").strip()
        if not message_id:
        ...
        # content 是需要逐值保存的 Chunk 原文。禁止 str()、strip() 或换行转换；空字符串
        # 仍是合法字符串，是否接受空来源由供应商协议解析阶段裁决。
        if not isinstance(self.content, str):
            raise TypeError("content must be str")
        if not file_name or not original_file_name:
            raise ValueError("source chunk file names cannot be empty")
        if not created_at:
            raise ValueError("created_at cannot be empty")
