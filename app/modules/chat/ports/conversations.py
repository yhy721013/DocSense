...
@dataclass(frozen=True)
class ChatSourceEvidence:
    """供应商无关的无损 Chunk 正文及结构化来源键。"""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.content, str):
            raise TypeError("ChatSourceEvidence.content 必须是 str")
        if not isinstance(self.structured_source_key, str):
            raise TypeError("structured_source_key 必须是 str")


@dataclass(frozen=True)
class ChatSourceFinalization:
    """常规回答的唯一完整来源终态。"""

    sources: tuple[ChatSourceEvidence, ...]

    def __post_init__(self) -> None:
        sources = tuple(self.sources)
        if any(not isinstance(item, ChatSourceEvidence) for item in sources):
            raise TypeError("sources 只能包含 ChatSourceEvidence")
        object.__setattr__(self, "sources", sources)


@dataclass(frozen=True)
class ChatQueryRejection:
    """Query 无上下文拒答的独立合法终态。"""
