...
class ChatCleanupJobExecutor:
    ...
    def execute_cleanup_job(self, *, job_id: str) -> ChatCleanupJob:
        ...
        if job.status == CLEANUP_JOB_SUCCEEDED:
            logger.debug(
                "文件对话清理任务已完成，跳过重复执行: job_id=%s conversation_id=%s",
                job.job_id,
                job.conversation_id,
            )
            return job
        ...
        logger.info(
            "开始执行文件对话清理任务: job_id=%s conversation_id=%s reason=%s attempt=%d",
            job.job_id,
            job.conversation_id,
            job.reason,
            job.attempt_count,
        ...
        try:
            if job.reason == CLEANUP_REASON_DELETE_CHAT:
                logger.info(
                    "执行会话删除后的远端资源清理: job_id=%s conversation_id=%s",
                    job.job_id,
                    job.conversation_id,
                )
                ...
                logger.info(
                    "执行临时标题线程清理: job_id=%s conversation_id=%s has_lease=%s",
                    job.job_id,
                    job.conversation_id,
                    bool(job.lease_id),
                )
            ...
            logger.warning(
                "文件对话清理任务失败并已保留重试记录: job_id=%s conversation_id=%s reason=%s attempt=%d error_type=%s",
                failed_job.job_id,
                failed_job.conversation_id,
                failed_job.reason,
        ...
        logger.info(
            "文件对话清理任务完成: job_id=%s conversation_id=%s reason=%s attempt=%d",
            completed.job_id,
    ...
    def _cleanup_deleted_chat(self, job: ChatCleanupJob) -> None:
        session = self._store.sessions.get(job.conversation_id)
        if session is None:
            raise ValueError("chat session does not exist for cleanup job")
        logger.debug(
            "加载待删除会话的资源租约: conversation_id=%s has_workspace=%s has_thread=%s",
            job.conversation_id,
        ...
        if not session.workspace_ref:
            unresolved = self._open_leases(job.conversation_id)
            if unresolved:
                logger.warning(
                    "删除会话清理无法继续：本地仍有未关闭租约但缺少工作区引用: conversation_id=%s open_lease_count=%d",
                    job.conversation_id,
                    len(unresolved),
                )
                raise ValueError("remote resource reference was not persisted")
            logger.info("删除会话清理无需调用远端资源: conversation_id=%s", job.conversation_id)
            return
        ...
        failed = self._failed_leases(job.conversation_id)
        if failed:
            logger.warning(
                "删除会话清理完成后仍有失败租约: conversation_id=%s failed_lease_count=%d",
                job.conversation_id,
                len(failed),
            )
    ...
    def _cleanup_temporary_thread(self, job: ChatCleanupJob) -> None:
        ...
        if lease.conversation_id != job.conversation_id:
    ...
    def _ensure_session_reference_leases(self, session: ChatSession) -> None:
        ...
        """
        logger.debug(
            "确保删除会话的资源租约完整: conversation_id=%s has_workspace=%s has_thread=%s",
        ...
        if session.workspace_ref:
            self._store.resource_leases.ensure_active(
                lease_id=chat_workspace_lease_id(session.conversation_id),
        ...
        if session.thread_ref:
            self._store.resource_leases.ensure_active(
                lease_id=chat_thread_lease_id(session.conversation_id),
                conversation_id=session.conversation_id,
    ...
    def _delete_main_conversation(
        ...
        *,
        ...
        if error:
            ...
            logger.warning(
                "文件对话主线程删除失败，等待工作区删除或后续补偿: conversation_id=%s error_chars=%d",
                conversation_id,
                len(error),
            )
            return
        self._store.resource_leases.mark_closed(lease_id)
        logger.info("文件对话主线程远端资源删除完成: conversation_id=%s", conversation_id)

    def _delete_context(self, *, conversation_id: str, conversation, context_ref: str) -> None:
        workspace_lease_id = chat_workspace_lease_id(conversation_id)
        self._mark_cleanup_pending_if_open(workspace_lease_id)
        for lease in self._document_binding_leases(conversation_id):
            self._mark_cleanup_pending_if_open(lease.lease_id)
        ...
        if error:
            ...
            for lease in self._open_leases(conversation_id):
                self._record_cleanup_failure_if_open(
                    lease_id=lease.lease_id,
                    error_message=error,
                )
            logger.warning(
                "文件对话工作区删除失败，已保留全部开放租约: conversation_id=%s error_chars=%d",
                conversation_id,
                len(error),
            )
            return
        ...
        for lease in self._store.resource_leases.list_by_chat(conversation_id):
            if lease.status != LEASE_CLOSED:
                self._store.resource_leases.mark_closed(lease.lease_id)
        logger.info("文件对话工作区远端资源删除完成，已关闭本地关联租约: conversation_id=%s", conversation_id)
    ...
    def _mark_failed(self, *, job: ChatCleanupJob, error: Exception) -> ChatCleanupJob:
        ...
        logger.info(
            "文件对话清理任务已安排重试: job_id=%s conversation_id=%s next_attempt_at=%s",
            failed.job_id,
    ...
    def _open_leases(self, conversation_id: str) -> tuple[ChatResourceLease, ...]:
        return self._store.resource_leases.list_by_chat(
            conversation_id,
            include_closed=False,
        )

    def _document_binding_leases(self, conversation_id: str) -> tuple[ChatResourceLease, ...]:
        return tuple(
            lease
            for lease in self._store.resource_leases.list_by_chat(conversation_id)
