...
from app.modules.chat.application.prompting import build_chat_title_prompt
...
class ChatTitleService:
    ...
    def generate_title(self, *, identity: ConversationIdentity) -> ChatTitleResult:
        ...
        if resolution is None:
            logger.info(
                "标题生成跳过: 对话不存在 identity_kind=%s",
                identity.identity_kind,
            )
            return ChatTitleResult(identity=identity, title="")
        normalized_conversation_id = resolution.conversation_id
        session = self._store.sessions.get(normalized_conversation_id)
        if session is None:
            raise RuntimeError("resolved conversation session is missing")
        if session.status != SESSION_ACTIVE:
            logger.warning(
                "标题生成被拒绝：会话当前不可用: conversation_id=%s status=%s",
        ...
        if not title_messages:
            logger.warning(
                "标题生成请求被拒绝: 对话历史为空 conversation_id=%s",
                normalized_conversation_id,
            )
        ...
        if not session.workspace_ref:
            logger.error(
                "标题生成失败: 对话缺少workspace引用 conversation_id=%s message_count=%d",
                normalized_conversation_id,
                len(title_messages),
            )
        ...
        logger.info(
            "开始生成文件对话标题: conversation_id=%s message_count=%d prompt_chars=%d",
            normalized_conversation_id,
            len(title_messages),
            len(prompt),
        )
        ...
        if not title:
            logger.error(
                "标题生成失败: 模型返回空标题 conversation_id=%s raw_chars=%d",
                normalized_conversation_id,
                len(str(raw_title or "")),
            )
            raise ChatTitleGenerationError("模型未生成标题")
        logger.info(
            "文件对话标题生成完成: conversation_id=%s title_chars=%d",
            normalized_conversation_id,
            len(title),
        )
    ...
    def _generate_with_tracked_temporary_thread(
        ...
        *,
        ...
        try:
            ...
            logger.info(
                "标题临时线程计划租约已创建: conversation_id=%s lease_id=%s",
                conversation_id,
                lease_id,
            )
        except ChatResourceLeaseSessionUnavailableError as exc:
            logger.warning(
                "标题生成被拒绝：创建临时线程前会话已不可用: conversation_id=%s status=%s",
                conversation_id,
                exc.status,
            )
            raise ChatTitleUnavailableError(
        ...
        try:
            with self._conversation_factory.create() as conversation:
                ...
                logger.info(
                    "标题临时线程已创建，准备记录租约身份: conversation_id=%s lease_id=%s",
                    conversation_id,
                    lease_id,
                )
                ...
                logger.info(
                    "标题临时线程租约已激活，开始请求模型回复: conversation_id=%s lease_id=%s",
                    conversation_id,
                    lease_id,
                )
                ...
                logger.info(
                    "标题临时线程模型回复已生成: conversation_id=%s lease_id=%s reply_chars=%d",
                    conversation_id,
                    lease_id,
                    len(raw_title),
                )
            ...
            logger.exception(
                "标题临时线程生成流程发生异常: conversation_id=%s lease_id=%s error_type=%s",
                conversation_id,
                lease_id,
            ...
            if temporary_session is None:
                ...
                cleanup_attempted = True
                logger.info(
                    "标题生成异常后开始补偿删除临时线程: conversation_id=%s lease_id=%s",
                    conversation_id,
                    lease_id,
                )
                try:
                    ...
                    logger.exception(
                        "标题临时线程补偿删除异常，准备记录可重试任务: conversation_id=%s lease_id=%s error_type=%s",
                        conversation_id,
                        lease_id,
            ...
            if temporary_session is not None and cleanup_error:
                logger.warning(
                    "标题临时线程未能立即清理，准备记录可重试任务: conversation_id=%s lease_id=%s error_chars=%d",
                    conversation_id,
                    lease_id,
                    len(cleanup_error),
                )
                ...
                logger.info(
                    "标题临时线程清理任务已调度: conversation_id=%s lease_id=%s job_id=%s status=%s",
                    conversation_id,
                    lease_id,
                    cleanup_job.job_id,
                    cleanup_job.status,
    ...
    def _record_temporary_cleanup_failure(
        ...
        *,
        ...
        logger.info(
            "标题临时线程清理失败已持久化: conversation_id=%s lease_id=%s job_id=%s",
            conversation_id,
            lease_id,
            cleanup_job.job_id,
        )
    ...
    def _dispatch_temporary_cleanup(
        ...
        """尝试一次立即清理，同时不掩盖持久化失败。"""
        logger.info(
            "开始内联尝试标题临时线程清理: job_id=%s conversation_id=%s",
            cleanup_job.job_id,
            cleanup_job.conversation_id,
        )
        try:
