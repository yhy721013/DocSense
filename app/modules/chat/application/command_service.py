...
from app.modules.chat.domain.models import ChatMessageSourceChunk, ChatRun
...
class ChatCommandService:
    ...
    def start_chat_run(
        ...
        *,
        ...
        logger.info(
            "准备启动 Chat 运行: identity_kind=%s message_chars=%d "
        ...
        logger.info(
            "文件对话运行已受理: conversation_id=%s run_id=%s status=%s "
    ...
    def reserve_chat_admission(
        ...
        *,
        ...
        logger.info(
            "Chat 请求已取得准入权: identity_kind=%s scope_mode=%s "
    ...
    def complete_chat_run(self, *, run_id: str) -> ChatRun:
        ...
        logger.info(
            "文件对话运行已成功完成: conversation_id=%s run_id=%s status=%s",
            run.conversation_id,
            run.run_id,
            run.status,
        )
    ...
    def fail_chat_run(self, *, run_id: str, error_message: str) -> ChatRun:
        ...
        logger.warning(
            "文件对话运行已标记失败: conversation_id=%s run_id=%s error_chars=%d",
            run.conversation_id,
            run.run_id,
    ...
    def abort_chat_run(self, *, run_id: str) -> ChatRun:
        ...
        logger.info(
            "文件对话运行已标记中断: conversation_id=%s run_id=%s abort_requested=%s",
            run.conversation_id,
            run.run_id,
            run.abort_requested,
        )
    ...
    def heartbeat_chat_run(
        ...
        *,
        ...
        logger.debug(
            "文件对话运行心跳已刷新: conversation_id=%s run_id=%s status=%s",
            run.conversation_id,
            run.run_id,
            run.status,
        )
    ...
    def request_abort(self, *, run_id: str) -> ChatRun:
        ...
        logger.info(
            "文件对话运行收到中断请求: conversation_id=%s run_id=%s abort_requested=%s",
            run.conversation_id,
            run.run_id,
            run.abort_requested,
        )
    ...
    def begin_chat_deletion(self, *, conversation_id: str) -> None:
        ...
        logger.info("开始请求文件对话删除准入: conversation_id=%s", conversation_id)
        self._run_coordinator.begin_chat_deletion(conversation_id=conversation_id)
        logger.info("文件对话删除准入已完成，会话已阻止新运行: conversation_id=%s", conversation_id)
    ...
    def issue_execution_lease(self, *, run_id: str) -> ChatRunLease:
        ...
        logger.info(
            "文件对话运行执行权已领取: conversation_id=%s run_id=%s has_owner_instance=%s",
    ...
    def validate_execution_lease(self, *, lease: ChatRunLease) -> ChatRun:
        ...
        logger.debug(
            "文件对话执行租约校验通过: conversation_id=%s run_id=%s status=%s",
            run.conversation_id,
            run.run_id,
            run.status,
        )
    ...
    def complete_chat_run_with_messages(
        ...
        *,
        ...
        if execution_lease is None:
            self._require_lease_or_allow_single_instance_compatibility()
            run = self._run_coordinator.complete_run_with_reply(
                run_id=run_id,
        ...
        logger.info(
            "文件对话运行已提交成功终态和消息: conversation_id=%s run_id=%s assistant_chars=%d",
            run.conversation_id,
            run.run_id,
    ...
    def fail_chat_run_with_user(
        ...
        *,
        ...
        logger.warning(
            "文件对话运行已提交失败终态: conversation_id=%s run_id=%s error_chars=%d",
            run.conversation_id,
            run.run_id,
    ...
    def abort_chat_run_with_user(
        ...
        *,
        ...
        logger.info(
            "文件对话运行已提交中断终态: conversation_id=%s run_id=%s",
            run.conversation_id,
            run.run_id,
        )
        return run

    def expire_stale_chat_runs(self, *, conversation_id: str) -> tuple[ChatRun, ...]:
        expired_runs = self._run_coordinator.expire_stale_runs_for_chat(
            conversation_id=conversation_id,
        )
        if expired_runs:
            logger.warning(
                "文件对话过期运行已在命令层收敛: conversation_id=%s run_ids=%s",
                conversation_id,
