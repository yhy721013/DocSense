...
from app.modules.chat.domain.models import ChatCleanupJob
...
class InlineChatCleanupDispatcher:
    ...
    def dispatch(self, *, job: ChatCleanupJob) -> ChatCleanupJob:
        ...
        logger.info(
            "开始内联调度文件对话清理任务: job_id=%s conversation_id=%s reason=%s attempt=%d",
            job.job_id,
            job.conversation_id,
            job.reason,
            job.attempt_count,
        ...
        try:
            ...
            logger.exception(
                "内联执行文件对话清理任务失败: job_id=%s conversation_id=%s reason=%s",
                job.job_id,
                job.conversation_id,
                job.reason,
            )
