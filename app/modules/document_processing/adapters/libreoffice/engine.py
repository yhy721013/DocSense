...
class _CommandTerminationUnconfirmed(RuntimeError):
    pass
...
class LibreOfficeLegacyOfficePreparer:
    ...
    def _convert(
        ...
        *,
        ...
        except _CommandTerminationUnconfirmed as exc:
            logger.error(
                "Legacy Office conversion failed "
                "code=conversion_timeout_outcome_unknown"
            )
            raise LegacyOfficeConversionError(
                "conversion_timeout_outcome_unknown",
                diagnostic=(
                    "LibreOffice conversion timed out and process-tree "
        ...
        *,
        ...
        if timed_out:
            if not termination_confirmed:
                raise _CommandTerminationUnconfirmed
    ...
    def _terminate_process_tree(self, process: Any) -> bool:
        ...
        if self._platform_name.startswith("win"):
            ...
            try:
                ...
                taskkill_succeeded = (
                    completed is None
                    or int(getattr(completed, "returncode", 0)) == 0
                )
        ...
        if process_group is not None:
            ...
            try:
                ...
                group_kill_succeeded = True
            except ProcessLookupError:
                # ESRCH 表示进程组已不存在，同样属于已停止确认。
                group_kill_succeeded = True
            except OSError:
                pass
        else:
