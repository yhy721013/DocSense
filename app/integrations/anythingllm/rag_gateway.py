...
class AnythingLLMRagGateway:
    ...
    def __init__(
        ...
        *,
        ...
        if embedding_max_attempts < 1:
            raise ValueError("embedding_max_attempts 必须大于 0")
    ...
    def _rollback_open_failure(
        ...
        *,
        ...
        attempts = [
            self._lifecycle_attempt(operation="open_context"),
            self._lifecycle_attempt(
        ...
        try:
            ...
            attempts.append(self._lifecycle_attempt(operation="rollback_context"))
            logger.info(
...
class AnythingLLMRagSession:
    ...
    def __init__(
        ...
        *,
        ...
        """保存任务级资源引用和原子 Client，不执行任何外部操作。"""
        self._document_client = document_client
        self._workspace_client = workspace_client
        self._thread_client = thread_client
        self._context_name = context_name
        self._context_ref = str(workspace.slug).strip()
        self._conversation_ref = str(thread.slug).strip()
        self._user_id = user_id
        self._embedding_max_attempts = embedding_max_attempts
    ...
    def close(self) -> CleanupResult:
        """最多执行一次工作区删除，并稳定复用首次清理结果。

        ``_closed`` 在发起删除前设置，确保删除请求超时或失败后，重复调用也不会盲目重放
        具有外部副作用的请求。上层可通过 ``already_closed`` 区分首次与重复调用。
        """
        if self._closed:
        ...
        try:
            ...
            result = CleanupResult(success=True, already_closed=False)
            logger.info(
                "anythingllm.session.cleanup: operation=close context_ref=%s "
    ...
    def _upload_document(self, file_path: str) -> AnythingLLMDocument:
        ...
        try:
            ...
            raise self._operation_error(
                self._safe_error(exc, fallback="文档上传响应不符合协议"),
                failure_stage="upload_protocol",
            ) from exc
        except Exception as exc:
            raise self._operation_error(
                self._safe_error(exc, fallback="文档上传失败"),
        ...
        if not document_id or not location or not document_ref:
            raise self._operation_error(
                "文档上传结果缺少有效 id、location 或 document_ref",
                failure_stage="upload_protocol",
            )
        logger.info(
    ...
    def _bind_document(self, location: str) -> None:
        ...
        for attempt in range(1, self._embedding_max_attempts + 1):
            try:
                ...
                raise self._operation_error(
                    self._safe_error(exc, fallback="嵌入响应不符合协议"),
                ...
                can_retry = (
                    ...
                    and attempt < self._embedding_max_attempts
                )
                if can_retry:
                    logger.warning(
                    ...
                    continue
                raise self._operation_error(
                    self._safe_error(exc, fallback="文档加入隔离上下文失败"),
    ...
    def _pin_document(self, location: str) -> None:
        ...
        for pin_attempt in (1, 2):
            try:
                ...
                raise self._operation_error(
                    self._safe_error(exc, fallback="文档 Pin 失败"),
                    failure_stage=failure_stage,
                ) from exc
            except AnythingLLMProtocolError as exc:
                raise self._operation_error(
                    self._safe_error(exc, fallback="Pin 响应不符合协议"),
