...
# Analysis 资源状态只服务仍在旧控制面的文件分析链；Report 已迁入 v2 专用组件，
# 不得为了复用常量重新把 Report Resource SQL 带回巨型 Task Service。
...
class LLMTaskService:
    ...
    def __init__(self, db_path: str):
        ...
        self._init_db()
        # 阶段 2-4 起三张 llm_interaction* 表的唯一物理 Writer 迁入共享
        # Observability Store。保留本 Service 方法仅作为 Analysis 完成 2-6 切换前的单向委托，
        # 禁止在此处恢复第二套 SQL 写路径。
        self._interaction_store = LLMInteractionStore(
            lambda timeout: self._connect(timeout_seconds=timeout)
        )
    ...
    def create_weaponry_task(
        ...
        Weaponry 生产受理已由阶段 2-5 的专用 Admission UoW 接管；旧 Service 不再
        创建或写入文档快照表，避免与 Task Control 组件形成第二 Writer。
        """
    ...
    def create_llm_interaction_with_trace(
        ...
        *,
        ...
        """
        return self._interaction_store.create_llm_interaction_with_trace(
            business_type=business_type,
            business_key=business_key,
            execution_id=execution_id,
            prompt=prompt,
            ...
            audit_idempotency_key=audit_idempotency_key,
            document_upload_facts=document_upload_facts,
        )
    ...
    def append_llm_interaction_lifecycle_events(
        ...
        *,
        ...
        """
        return self._interaction_store.append_llm_interaction_lifecycle_events(
            interaction_id,
            events,
            cleanup_status=cleanup_status,
            cleanup_error=cleanup_error,
            expected_execution_id=expected_execution_id,
            expected_audit_idempotency_key=expected_audit_idempotency_key,
        )
