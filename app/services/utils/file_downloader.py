...
# 遗留 analysis/report 调用未显式传入上限时仍必须有安全边界。报告新链路会从严格配置
# 传入同一默认值；该值只限制单个远端文件，不限制持久任务积压数量。
...
def download_to_temp_file(
    ...
    """在总时限和大小上限内流式下载，并通过原子替换发布完整文件。

    ``requests`` 的单个 timeout 只限制连接/某次读取等待，并不是完整传输期限。这里使用
    短的单次 I/O 超时配合 monotonic 总截止时间，防止慢速持续响应永久占用唯一报告
    Worker。部分文件始终写入随机 ``.part``，异常时删除，调用方不会读到半文件。
    """
    ...
    if (
        ...
        or total_timeout in (float("inf"), float("-inf"))
        or total_timeout <= 0.0
    ):
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "清理下载部分文件失败: file_name=%s",
                safe_name,
                exc_info=True,
            )
        logger.warning(
            "文件流式下载失败: file_name=%s downloaded_bytes=%d max_bytes=%d",
            safe_name,
            downloaded_bytes,
            max_bytes,
            exc_info=True,
        ...
        if response is not None:
            try:
                ...
                # 响应清理失败不能覆盖前面更有价值的下载异常，也不能把已经原子发布的
                # 完整文件伪装为下载失败；连接池异常通过日志交给运维观察。
                logger.warning(
                    "关闭下载响应失败: file_name=%s",
                    safe_name,
                    exc_info=True,
                )

    logger.info(
        "文件流式下载完成: file_name=%s bytes=%d max_bytes=%d",
        safe_name,
        downloaded_bytes,
        max_bytes,
    )
...
__all__ = ["DEFAULT_MAX_DOWNLOAD_BYTES", "download_to_temp_file"]
