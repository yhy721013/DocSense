...
def build_file_analysis_prompt(request_params: dict) -> str:
    ...
    return (
        ...
        "4. country/channel/maturity/secrets/format 只能输出候选项中的 value 字符串；secrets 表示文件密级，必须根据文档开头、首页、页眉或标题附近的密级/保密说明判断，找不到相关说明时输出“公开”；fileDataItem.dataFormat 必须与顶层 format 完全一致，也只能输出格式候选中的 value；不能输出 key，也不能输出对象。\n"
        ...
        + data_standard_priority
        + "【抽取字段解释】secrets：文件密级，只能从密级候选 value 中选取；优先依据文档开头内容判断，文档没有密级、保密、公开等相关说明时默认为“公开”。keyword：文档中提到的关键信息或主题，由至少 10 个关键词构成，关键词需要涵盖文章中提到的内容，按照占比从高到低排列；score：资料来源权威性评分；source：文档中提到的具体数据来源出处，缺少明确出处时输出“未明确数据来源”；fileNo：文件编号；dataFormat：资料格式，必须与顶层 format 完全一致，并且只能使用格式候选中的 value。\n"
        + "【输出前自检清单】\n"
        + "1. country/channel/maturity/secrets/format 是否都为候选 value 或空字符串；secrets 缺少文档开头密级说明时是否已输出“公开”；fileDataItem.dataFormat 是否与顶层 format 完全一致。\n"
