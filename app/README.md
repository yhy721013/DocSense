...
| `adapters/` | 应用边界适配器；阶段 1A 已建立 Flask 入站解析命名空间，尚未迁移正式路由。 |
| `modules/` | 按业务聚合的模块化单体根；阶段 1A 已建立 `tasks` 的 DTO、Port、应用服务、分层骨架和依赖门禁。 |
...
- `modules/tasks` 当前具备经 Fake 测试的内部契约，但不代表 check-task/Progress 已经迁移；生产调用链仍由现有 Blueprint、Task Service 和 Progress Hub 承担，直至波次 1B 明确切换。
