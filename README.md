...
- **MHTML 支持**：甲方正式接口 `/llm/analysis` 与 `/llm/generate-report` 支持 `mhtml`/`mht` 文件，下载后会先在服务端提取正文文本再进入现有大模型处理流程
...
- `/llm/analysis` 对 `mhtml`/`mht` 文件会先提取正文文本并生成临时标准文本文件，再复用现有解析流程。
- 多文件请求不会并行解析，而是同一批次内串行处理：前一个文件结束后才开始下一个文件。
- `/llm/generate-report` 的 `filePathList` 可包含 `mhtml`/`mht` 文件，服务端会先提取正文文本再参与报告生成。
